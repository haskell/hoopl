...
about a continuation.\simon{Shall we mention that typically forward
...
Getting spills and reloads in the right places requires\simon{``Requires'' is too strong.
This is simply how we have chosen to implement it.  You are making it sound
ineluctable.} the
composition of several dataflow passes:
\begin{enumerate}
\item
A backward transformation inserts reloads immediately after each call
site.\simon{It is backward because it is doing a live-variable analysis
to identify which variables should be reloaded.}
...
pressure.\simon{Why is the second pass a second pass?  The first
...
\secref{dead-code-elimination}. \simon{I would put the forward references
...
\simon{How can the sltack slot be ``out of date''?  
There are two ways:
x might change, or the stack slot might change.  However, by keeping the
stack slot live whenever x is, we arrange that only the former can happen,
...
because such saves immediately preced calls?}}
