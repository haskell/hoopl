...
A special case is an assertion about the state at a program point,
which is established by the paths leading from the entry to the program point.
...
Assertions about state are easy to formalize (e.g. the assertion @x == 7@ 
in \secref{constant-propagation}), but path properties are harder;
