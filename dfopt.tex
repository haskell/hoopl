...
and similarly for graphs and nodes.)
...
to contain an arbitrary non-empty list of @Block@s, each closed at both ends.
\john{Shouldn't we clearly identify this as a dynamic invariant?}
The signature for @GMany@ uses Haskell's named-field syntax to say that
...
transfers control to one of the @g_blocks@. In contrast, a graph closed on entry
...
\end{code}
This definition is a lovely example of the power of GADTs: the
...
\end{figure}
If \ourlib{} knows nothing about nodes,
how can it know what is the @BlockId@ at the start of a block,
or to which block a control transfer goes?
...
the fact is (@x=7@, @y=4@, @z=True@).  Joining these gives
...
\john{Previous comment is probably obsolete.}
...
on entry, then the incoming thing is an @InFactC f@,
which is just a function from @BlockId@ to @f@.
The same technique makes the result type of @ForwardTransfers@ vary with
the exit flag @x@.
