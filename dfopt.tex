...
To summarise, suppose E is a node, block, or graph: \simon{Removed the claim
...
in  \figref{graph}.
Like, @Block@ the data type @Graph@ is parameterised over
...
\begin{tabular}{ll}
O/O & @GUnit b@ \\
O/C & @GMany (OpenLink b) BodyEmpty    ClosedLink@ \\
C/O & @GMany ClosedLink   BodyEmpty    (OpenLink b)@ \\
C/C & @GMany ClosedLink   (BodyUnit b) ClosedLink@
\end{tabular}
\end{center}
...
type FactBase f = LabelMap f
 -- A finte mapping from Labels to facts f
\end{code}
...
optimised @Body@.  One thing that is immediately obvious from this
...
\simon{Fixed now?  Or still a problem?}
...
\begin{code}
  transfer_fn :: ForwardTransfer n f
  node        :: n e x
\end{code}
...
\begin{code}
type AGraph n e x 
  = LabelSupply -> (Graph n e x, LabelSupply)
...
\par{\small
\begin{code}
`withLabel :: (Label -> AGraph n e x) -> AGraph n e x
...
The definition of @combineFRW@ (which is provided by Hoopl although we show
it in \figref{const-prop}) is interesting: it says to try @r1@, and if that 
fails try @r2@.  We cannot apply @r2@ directly to the result of a successful
rewrite with @r1@, because @r1@ returns a @Graph@ while @r2@ expects a node.
Instead we ask \ourlib{} to re-apply the transform by asking for @DeepRewriting@.
...
Finally, we invoke the analysis by calling  @analyseAndRewriteFwd@.
...
\begin{code}
`normaliseBody :: Edges n => RG n f C C 
               -> (Body n, FactBase f)
\end{code}
