...
the previous state.  \simon{What is @set\_or\_save@?}
...
These monads are \simon{``This monad is''?} private to the implementation of the library;
...
Function @solve@ initializes the inner dataflow monad by setting the fact on
the entry label to @in_fact@ (\lineref{setFact.g.entry}), then
applying a higher-order @run@ function
(not~shown), 
...
Each block comes from  the list of reachable blocks in @blocks@~(\lineref{solve.blocks}), 
which is computed by reverse postorder depth-first traversal.
(Any order will get us a fixed point,
but reverse postorder depth-first order hastens convergence of forward dataflow
...
Function @solve_tail@ walks the block and computes @last_outs@, 
of type @LastOuts a@ from \figref{transfers},
which
...
% space is needed but not here, because the start of section 7.3 sets
% a strong page break. ---NR
