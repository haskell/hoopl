...
This polymorphism is what makes our library work with any intermediate
language, but there is a constraint: @(LastNode l)@ means that @l@~may
be instantiated only with a type providing a function 
\begin{code}
  succs :: l -> [BlockId]
\end{code}
which gives the identifiers of the blocks to which a node of type~@l@
might transfer control.
\remark{Would like to have a more limited constraint
...
{The meaning of the final type parameter~@()@ is 
...
\item
Rewriting is monadic; the @FuelMonad@ holds resources needed to
...
\end{itemize}
\remark{Using @withDuplicateFuel@ is a hell of a lot simpler than the
``fuel adminstration'' in our caml code}
