...
and we move on to the next node.  If the rewrite function returns @(Just g)@,
indicating that it wants to rewrite the node to the graph @g@, then
we recursively analyse and rewrite @g@ before moving on the the next node.
Notice that the nodes following the rewritten node see 
...
rewrite function; it is not a global flag.
We express this idea elegantly in the type of @FwdRewrite@
