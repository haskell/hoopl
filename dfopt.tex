...
We spend less space, in Sections
\ref{sec:graphs}~and~\ref{sec:engine}, on the shared implementation.
...
Each type of node has its own invariant, which
constrains the number of predecessors and the number of successors of
that type of node in the control-flow graph.
...
Our view of program analysis is grounded in the seminal work done by
...
body implies $i=2$.
We therefore have to recompute based on a precondition of $i=1 \join
i=2$.
...
abstract treatments found in textbooks (\secref{gen-kill}).
\item
