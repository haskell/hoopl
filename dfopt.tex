...
regHasConst :: ForwardTransfers Node ConstFact
regHasConst f (Label bid)            = f
...
regHasConst f (Branch bid)           = [(bid, f)]
...
constProp facts n =
  mapE rewriteE n >>= return . toAGraph
  where
    rewriteE e@(Reg r) =
...
simplify :: ForwardRewrites Node f
simplify _ node = s node >>= return . toAGraph
  where
    s :: Node e x -> TxRes (Node e x)
    s (Cond (Bool b) t f) = Just $ Branch (if b then t else f)
    s n                   = mapE exps n
    exps (Binop Add (Int i1) (Int i2)) ->
       Just $ Int $ i1 + i2
    ....  -- more cases for constant folding

-- Rewriting expressions
mapE :: (Expr -> Maybe Expr) -> Node e x -> Maybe (Node e x)
mapE f   (Label _)    = Nothing
mapE f n@(Assign x e) = fmap (Assign x) $ f e
 ... more cases for rewriting expressions

-- Combining transformations
combine :: (f -> n -> Maybe g) -> (f -> n -> Maybe g)
        -> (f -> n -> Maybe g)
