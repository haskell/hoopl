...
from many years' work on code improvement, also called
``optimization.''
...
Typical presentations obscure fundamental principles of
code improvement.
...
avail_vars_transfer = ForwardTransfers (flip const) middleAvail lastAvail
...
\remark{Check these bullets against the figure for correctness}
\begin{itemize}
\item
...
the transformation
and
analysis in two phases \citep{lerner-grove-chambers:2002}:\seclabel{solver-phase}
\begin{itemize}
\item
...
\remark{We might wish to explain (briefly) how this algorithm could possibly work.
E.g., When you analyze a written graph, you could get a fact that's lower,
but when you reach a join point, you take the join, and the fact
doesn't get smaller.  So things terminate. (And why is rewriting sound?)}
...
The rewrite function is like the @zdfSolveFwd@ function in
...
\item
The rewriter requires rewrite functions as well as transfer
functions.
\item
...
in Figure 3?  Instead we pass the transfer functions @avails\_vars\_transfer@
...
engine (lattice, transfer functions, rewrite functions) is simple,
