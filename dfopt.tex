...
We~enforce this restriction by using the open/closed flags we have
introduced:\remark{I'm trying to signpost this as an innovation}
@BCat@
...
has at least one node.\remark{So what? Who cares?  Why is it important
that no block is empty?  (And if we had an empty block, we could ditch
@PNil@ from @PGraph@, which woudl be an improvement.)}


A @Block@ closed at both entry and exit is what compiler writers normally 
call a ``basic block.''
In the case of @Node@, for example,
the types elegantly ensure that a block of type \mbox{@Block@ @Node@ @C@ @C@}
begins with a @Label@, 
continues with a (possibly empty) sequence of @Assign@s and @Store@s,
and finishes with a @Branch@ or @CondBranch@.  
...
graph:\remark{This analogy should be developed early and often}
\begin{itemize}
...
of a full function/procedure/method}
