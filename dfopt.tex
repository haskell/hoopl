\input{dfoptdu} % use version with def-use information
...
but by using % Our new implementation, which uses generalized
generalized algebraic data types % \cite{xi:guarded-recursive} 
...
The professional literature on general dataflow optimization is primarily
about foundations or about particular analyses, not about reusability
or implementation technique. 
Muchnick's \citeyearpar{muchnick:compiler-implementation} textbook
introduces both fundamental principles and several important analyses.
In the most closely related work, 
\citet{kildall:unified-optimization} showed that iterative dataflow analysis computes a
meet-over-all paths solution to recursive dataflow equations.
\citet{schmidt:dataflow-analysis-abstract-interpretation} showed that
an all-paths dataflow problem can be viewed as model checking an
abstract interpretation of the program.
\citet{lerner-grove-chambers:2002} showed that their iterative
algorithm for 
interleaving analysis with rewriting is sound, even when some
speculative rewrites might not be performed on later iterations.
\remark{Kam and Ullman showed that iterative analysis computers
maximum fixed point; Kildall showed that if transfer functions are
distributive this is equal to the meet-over-all-paths solution.
(Strong distributivity: $f(x \sqcup y) = f(x) \sqcup f(y)$
and $f(x \sqcap y) = f(x) \sqcap f(y)$.)
In Hoopl we ignore the meet operation, but the theorem holds.}
...
\remark{As usual, transfer functions must be monotone}
\end{itemize}
...
Both the foundations of dataflow and  particular analyses are covered
by a vast literature;
design of implementations, the topic of this paper, is covered
relatively sparsely.
...
If~the lattice's join operation distributes over all transfer
functions,
this fixed point is equivalent to a join-over-all-paths solution to
the recursive dataflow equations \cite{kildall:unified-optimization}.
(Kildall's paper uses meets, not joins.  The orientation of the
lattice is conventional, and since 1973 conventions have changed.
We use Dana Scott's
ordering, in which higher elements correspond to more information.)
\cite{kam-ullman:monotone-flow-analysis} generalize the result some
monotone functions.
In~Hoopl, it is the client's responsibility to ensure that all
transfer functions are monotone, but for transfer functions that
approximate weakest preconditions or strongest postconditions,
monotonicity falls out automatically.
...
Our implementation uses the algorithm of
\citet{lerner-grove-chambers:2002}, which they showed to be sound even
though some
speculative rewrites might not be performed on all iterations.
...
\newcolumntype{L}{>{\T\Large\bfseries}m{1.3in}<{\centering}}       % label
...
% omit isStackSlotOf :: CmmExpr -> LocalVar -> Bool
% omit isStackSlot :: CmmExpr -> Bool
% omit variableOfSlot :: CmmExpr -> LocalVar
...
%
% omit delFromVarSet :: VarSet -> LocalVar -> VarSet
% omit extendVarSet :: VarSet -> LocalVar -> VarSet
% omit unionVarSets :: VarSet -> VarSet -> VarSet
% omit sizeVarSet :: VarSet -> Int
% omit emptyVarSet :: VarSet
% omit unionManyVarSets :: [VarSet] -> VarSet
% omit isEmptyVarSet :: VarSet -> Bool
% omit elemVarSet :: LocalVar -> VarSet -> Bool
% omit VarSet :: *
...
\end{code}
% omit varSetToList :: VarSet -> [LocalVar]
\B
...
reload. 
%
% omit filterVarsUsed :: UserOfLocalVars e => (LocalVar -> Bool) -> e -> VarSet
%
...
!solve.ex.sig!   where `solve_ex    :: GF m l e x -> ZMaybe e a -> DFM a (ZMaybe x a)
...
% @solve_first@, @solve_mid@, and @solve_last@
...
% instead of using ML's built-in state monad, 
which
