...
(c) optimization fuel, (d) computing fixed points.
...
and an input fact,
and returns a replacement graph of type @(RG n e x)@ of the same shape
as the @thing@, and a suitably shaped output fact.

Regardless of whether @thing@ is a node, block, or graph, the result
is always a graph.
We represent this graph using a new type~@RG@,
rather than @Graph@, for two reasons:
\begin{itemize}
...
\end{itemize}
The normalization function expresses \remark{``expresses''?} 
the two points above by splitting 
a replacement graph into a @Graph@ and its corresponding @FactBase@:
\begin{code}
...
Here is where we must take care of fixed points (\secref{arf-body}).
...
In the @Just@ case, the rewrite function returns a replacement
...
Lastly, @arbBody@ deals with the fixed-point calculation.
...
of each of the successively analyzed blocks.  The @fixpoint@ algorithm
...
It is easy for the fixed-point algorithm to take account of this.  Recall
...
appear in the replacement graph.
