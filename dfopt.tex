...
\title{\ourlib: a modular, re-usable library for \\ dataflow analysis and transformation}
...
\item \ourlib{} implements a rather subtle algorithm, involving at least
(a) interleaved analysis and rewriting, (b) speculative rewriting,
(c) optimisation fuel, (c) fixpoint finding.
...
\remark{Maybe the title should hearken to
...
It's worth noticing that the arguments of @ForwardPass@ are, and must be,
polymorphic functions, so \ourlib{} makes essential use of higher-rank types.
...
\item Writing a dataflow optimiser for an imperative language
...
including:
optimisation fuel in \secref{fuel}; 
