...
$@x == 7@ \land @y == 8@$ (call it $f_{\mathit{new}}$) along the edge to~@L2@.
...
(call it $f_{\mathtt{L2}}$), with the lattice join $f_{\mathit{new}} \join f_{\mathtt{L2}}$.
...
$f \join f_{\mathit{id}} = f_{\mathit{id}}$ at every label~$\mathit{id}$.
When the facts associated with labels no longer change, the dataflow
