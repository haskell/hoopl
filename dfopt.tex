...
!solve.rewrite.shallow.1!                     RewriteShallow -> do { a <- anal_f_OO g in'
!solve.rewrite.shallow.*!                                          ; return (a, fuel) }
!solve.mid.*!              ; k a fuel }
\end{numberedcode}
%%  
%%  
...
function @solve_blocks@ takes a list of blocks
produces a continuation of type @FuelKont a Fuel@, which
iterates @solve_block@ until it reaches a
fixed point.
Using @solve_blocks@, we write solvers for subgraphs which are open/open,
...
dataflow monad.
It also returns the remaning fuel, and if it is open at the exit, it
returns an exit fact as well.

Ultimately, \emph{all} solver functions
are composed of applications of @solve_first@,
@solve_mid@, and @solve_last@: they are the base cases.
All three are similar, so we show just one:
@solve_mid@ appears on
...
(\linepairref{solve.rewrite.shallow.1}{solve.rewrite.shallow.*}).
\end{enumerate}
