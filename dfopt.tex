...
pervasive in optimising compilers, but it is generally tightly
interwoven with the details of a \emph{particular} compiler.  We
describe \ourlib{}, a re-usable Haskell library that makes it
unusally easy for compiler writers to define new analyses and
transformations.  The API of our library is modular, polymorphic,
and offers unusually strong static guarantees.  The implementation
is also far from routine: it encapsulates 
state-of-the-art algorithms (interleaved analysis and rewriting,
dynamic error isolation), and cleanly separates their tricky elements
...
rewriting, optimization fuel, and so on) are implemented all together,
inseparably. \remark{Maybe the title should hearken to
...
the code in a separate git repo---but we'll get there.} \simon{I positively
want the paper and the code to travel together so that we can update the paper
(properly attributed of course) as we update the code.}
...
To summarize, whenever E is a node, block, or graph,
...
sequence.\remark{I've changed ``link'' to ``sequence'' here and here
alone.  If J\&S agree, I~can make the rest of the paper consistent.}
\begin{itemize}
\item 
...
gCat (GMany e1 bs1 (JustO x1)) 
            (GMany (JustO e2) bs2 x2)
...
analyse-and-rewrite function can transform a @Body@ into an
...
and deals with fuel consumption.  It calls @arfGraph@ to analyse
...
\end{code}
We have spoken only of \emph{forward} analyses and transformation, and we will
continue to do so.  However, 
\ourlib{} provides an exactly analogous family 
backward-analysis functions (\secref{overview}).
 
\subsection{From nodes to blocks} \seclabel{arf-block}
\seclabel{arf-graph}

We start with the second task, namely analysing and transforming blocks.
...
optimisation fuel (\secref{fuel}), @withFuel@ returns @Nothing@.
...
\end{code}}
and analyse the resulting graph with @arfGraph@.  But before
...
shallow rewriting, the rewrite function should return @noFwdRewrite@
in the @FwdRes@ (\secref{rewrites}).
...
(the fourth argument), and keep doing so until a fixed point is
reached.  The code for @fixpoint@ is a whole 70 lines long, and
appears for completeness in Appendix~\ref{app:fixpoint}.  We highlight some interesting
points here:
\begin{itemize}
\item Incomplete
\end{itemize}
...
\appendix \label{app:fixpoint}

\section{Code for \texttt{fixpoint}}
...
updateFact :: DataflowLattice f -> LabelSet -> (Label, f)
           ...
           -> (ChangeFlag, FactBase f)
-- See commment about tfb_cha above
...
fixpoint :: forall n f. Edges n =>  DataflowLattice f
         -> (Block n C C -> FactBase f
              -> FuelMonad (RG n f C C, FactBase f))
...
fixpoint lat do_block init_fbase blocks
  = do { fuel <- getFuel  
       ...
       ; return (tfb_blks tx_fb, 
                 tfb_fbase tx_fb `deleteFromFactBase` blocks) }
  ...
  where
    ...
    tx_blocks ((lbl,blk):bs) tx_fb 
        = do { tx_fb1 <- tx_block lbl blk tx_fb
             ; tx_blocks bs tx_fb1 }
