...
\begin{abstract}
We present an interface that makes it easy for compiler writers
...
must be given a lattice structure such that every assertion can be increased at
...
``optimization.''
But the way this work is typically presented annoys us: 
...
The typical treatment obscures fundamental principles of
code improvement.
...
We consider code improvements only over low-level
...
We spend less space, in \secref{engine}, on the shared implementation.
...
assertions about a variable~$x$ \simon{I'm hazy about when you
want to say ``fact'' and when  you want to say ``assertion'', or indeed
if you intend to distinguish the two at all.}:
\begin{itemize}
\item
...
We~notate this assertion as $x = \top$.
\item
...
The client must choose how to represent these assertions. In this
case, remembering the the assertion at each edge is a conjunction of assertions
about each in-scope variable, an appropraite choice might be a finite mapping
\begin{code}
  type CPFact = Data.Map Var (Maybe Const)
\end{code}
If the variable $x$ is not in the domain of the map, then $x=\bot$;
if it maps to @Nothing@ then $x=\top$; if it maps to $(@Just@~k)$ then
$x=k$.
...
The constraints on the graph structure determine the signatures of
transfer functions, 
...
\simon{Notice that although the
library fixes the \emph{structure} of the graph, it is the client that
specifies the \emph{content} of middle and last nodes.}
\norman{We are having trouble understanding SLPJ's obsession with distinguishing the ``client'' (an entity we wish not to name) from the ``library.''}
...
the
identity function.\norman{It is not @flip const@ when we do the proc-point
analysis, but I am not exactly sure how to work this information into
the text.}  
...
\simon{I don't think so.  What is $f$?}
...
graph;
because the analysis may be run on a graph returned by a rewrite
function,\remark{forward ref, maybe}
this fact
is not always~$\bot$.
...
The most significant information includes
