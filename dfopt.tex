...
The primitive constituents of a \ourlib{} control flow graph are \emph{nodes},
reprsented by a data type defined by the client.  Typically, a node might represent a machine 
instruction, such as an assignment, a call, or a conditional branch.
...
Now that we have graphs in hand, we are 
ready to return to dataflow optimisation.
...
\emph{dataflow facts}.
Dataflow facts relate to traditional program logic:
\begin{itemize}
\item
...
the control-flow graph, where an edge connects two nodes.
%%\footnote
...
\item
Write a \emph{transfer function} that relates the dataflow facts before and
after a node (\secref{tffuns}).
\end{itemize}
...
Nodes in a control-flow graph & You & You & One datatype per intermediate language \\[3pt]
Dataflow fact~$F$    & You & You & One datatype per logic \\
Lattice operations & Us & You & One set per logic \\[3pt]
Transfer function & Us & You & One per analysis \\
Rewrite function & Us & You & One per \rlap{transformation} \\[3pt]
...
operations, transfer function, and rewrite function
to one of \ourlib's
...
% omit fact_name :: DataflowLattice a -> String
\else, 
...
\begin{figure}
\begin{code}
type InFacts  f = BlockId -> f
type OutFacts f = [(BlockId, f)] 

type ForwardTransfers n f 
  = forall e x. n e x -> InF e f -> OutF x f 

type BackwardTransfers n f 
  = forall e x. n e x -> InF x f -> OutF e f 

type family   InF e f :: *
type instance InF O f = f
type instance InF C f = InFacts f

type family   OutF x f :: *
type instance OutF O f = f
type instance OutF C f = OutFacts f
\end{code}
...
But wait!  What if the node is closed on entry or exit?
...
produce a list of (@BlockId@,fact) pairs, one for each outgoing edge.  So 
the types of the transfer function's argument and result each 
...
an incoming fact-like thing of type (@InF@ @e@ @f@), and produces an
outgoing fact-like thing of type (@OutF@ @x@ @f@).  The type constructor @InF@
...
on entry the incoming thing is an @InFacts f@, which is just a function from @BlockId@
...
this type.  We will give a concrete example shortly, 
in \secref{example-analyses}.  The supplied transrer function should be 
...
\subsection{Running the dataflow engine} \seclabel{zdfSolveFwd}

\iftrue
Once the client has written the lattice operations and the
transfer function, the \ourlib{} dataflow engine lifts
the transfer function from nodes up to graphs.  Here is the signature
for the forward analyser
(there is a backward analyser with a similar type):
\begingroup\hfuzz=1pt\relax
\begin{code}
 `analyseFwd 
  :: Edges n    
  => DataflowLattice f      -- Lattice
  -> ForwardTransfers n f   -- Transfer functions
  -> Graph n e x            -- Control-flow graph
  -> InF e f                -- Input fact
  -> OutF x f
\end{code}
The similarity to the @ForwardTransfers@ type should be clear: 
@zdfSolveFwd@ is 
just a transfer function for whole graphs.
...
\T\hfuzz=45pt % overlap is better than poor indentation
\begin{code}
...
%%  \hline
%%  
\T\hfuzz=87pt % flagrant cheating
\begin{code}
...
\section{Example analysis passes} \seclabel{example-analyses}

\simon{Start of note}
{\em I'm not very comfortable with this section.
We really only give the details for pure analysis, but the words 
only describe an analysis/rewrite story.
Do you actually
envisage doing much pure analysis, separated from rewrites? Yes, I think
so; in GHC the proc-point analysis or stack layout is, I believe, separate.
But for a pure analysis we presumably want a (BlockEnv Fact) for every block,
not just the externally visible blocks.  So we want something like
\begin{code}
  pureanal :: .. -> Graph C C -> BlockEnv Fact
\end{code}
But maybe it would be more honest to say this?
\begin{code}
  pureanal :: .. -> [Block C C] -> [Fact]
\end{code}
None of this is hard to implement. But the API just isn't clear to me yet.
}
\simon{End of note}

\simon{An alternative in this section about analysis 
would to give the transfer functions
for a standard constant-prop (forward) and liveness (backward) analysis,
without trying to link it to the larger GHC story.  The link is weaker now,
because we are describing something whose GHC incarnation is for a much
earlier version of Hoopl; and the focus of the paper is much more on the
cool API and GADTs etc, so the ``works at scale'' story isn't so vital.}
...
we present examples that have been implemented and tested in GHC.  
\simon{Not quite true...}
GHC's low-level intermediate code, called @`Cmm@, is a 
subset of the portable assembly language~{\PAL}
\cite{peyton-jones-ramsey:single-intermediate}.
We~specialize \ourlib\ to GHC by instantiating \ourlib's node
parameter to the @CmmNode@ type introduced in \figref{cmm-node}.
...
\T\hfuzz=7pt % plenty of room where it matters
\begin{code}
`liveTransfers :: BackwardTransfers CmmNode Live
liveTransfers (Label bid)     	       f  = [(bid,f]
liveTransfers (Assign r e)    	       f  = remDefd r (addUsed e f)
lifeTransfers (Store e1 e2)   	       f  = addUsed e1 (addUsed e2 f)
liveTransfers (Branch bid)             inf = inf bid
liveTransfers (CondBranch e bid1 bid2) inf = addUsed e (unionLive (inf bid1) (inf bid2))
\end{code}
...
\T
\begin{code}
cmmLiveness :: Graph CmmNode O C -> BlockEnv Live
...
We begin with a simple liveness analysis, a backward analysis 
that supports passes 1~and~3. 
The assertion computed by 
a backward dataflow analysis applies to a
\emph{continuation} at a program point.
In the case of liveness analysis, \emph{the} classic example of 
backwards analysis,
...
The transfer function for liveness relies on two auxiliary functions
...
For a last node, the tranfer function consults the @InFacts@
function @inf@ (on \lineref{lastLiveOut.1}) to find out what
...
Notice that the type of the second parameter varies
according to whether the node is closed on exit; and the type
of the result varies according to whether the node is closed on entry.

Given the lattice and the transfer functions,
we perform liveness analysis on a whole graph by calling
...
% omit varOfSlot :: CmmExpr -> LocalVar
...
\ifpagetuning
\enlargethispage{1.0\baselineskip}  % balance ``A middle node stores ...''
\fi
...
\seclabel{rewrites} \seclabel{example-rewrites}
...
type ForwardRewrites n f 
  = forall e x. n e x -> InF e f -> Maybe (AGraph n e x)
type BackwardRewrites n f 
  = forall e x. n e x -> InF x f -> Maybe (AGraph n e x)
\end{code}
...
For example, knowing that a register is dead allows
us to rewrite an assignment to that register by a no-op.  Knowing that a register is
live at a call site might provoke us to insert a spill before the call.  And so on.

In general, we want to replace a node not just by a single node, nor even by a block,
but by an arbitrary graph.  For example, perhaps we want to replace a high-level operation
...
but rather an @InFacts f@, using the same @InF@ function as described
...
\subsection{Running the dataflow engine} \seclabel{running-rewriter}

\overfullrule=10pt

To use the \ourlib{} dataflow engine to transform a control-flow graph, you
use the function @analyseAndRewriteFwd@ 
(there is a similar function for backward analysis):\begingroup\hfuzz=1pt\relax
\begin{code}
 `analyseAndRewriteFwd
  :: Edges n         
  => DataflowLattice f      -- Lattice
  -> ForwardTransfers n f   -- Transfer function
  -> ForwardRewrites n f    -- Rewrite function
  -> RewritingDepth         -- Rewrite recursively?
  -> Graph n e x            -- Graph or subgraph
  -> InT e f                -- Input fact(s)
  -> FuelMonad (Graph n e x,  -- Rewritten graph
                OutFact e f)  -- Output fact(s)
\end{code}
The following points are worth noting about this type:
\begin{itemize}
\item
The function takes a rewrite function, as well as a 
transfer function and dataflow lattice.
\item
The @`RewritingDepth@ parameter controls recursive rewriting, in a manner
to be described shortly.
\item
The function transfoms a @Graph@ and a input fact of the appropriate shape,
into a rewritten graph and a corresponding output fact.
\item
...
The user-level model of how @analyseAndRewriteFwd@ works is this.
The function walks forward over the graph, computing dataflow facts
just as in the pure analysis case.  At each node, it applies
the rewrite function to the node and its incoming fact.  If the 
rewrite function return @Nothing@, the node is retained as part of the output
graph, and we move on to the next node.
...
Facts computed by the @analyseAndRewriteFws@ depend on graphs produced by rewrite
...
\T\hfuzz=21pt % lots of cheating on code widths here
\begin{code}
...
\begin{figure*}\hfuzz=1pt
\begin{codetable}\hfuzz=21pt % lots of cheating on code widths here
\T
\begin{code}
...
\T\hfuzz=40pt % we know it's too wide, thanks
\begin{code}
...
\simon{From here on is old stuff}
