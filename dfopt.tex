...
without constant folding!  And pobably vice versa.}
...
examples.
\simon{I have improved a bit, I think. If not enough, can you reword?}
...
More generally, we will often informally 
say ``a closed/open node'' 
rather than ``a node of type (@CmmNode@ @C@ @O@)'', 
and simlarly for blocks and graphs.
...
A @Block@ 
is parameterised over the node type @n@
...
Meanwhile, it may also be helpful to see how @analyseAndRewriteFwd@ can
...
\begin{figure}
\begin{code}
data HasConst = Top | B Bool | Int Integer
...
varHasConst (Cond (Bool b) tid fid) f =
  [(tid, Map.insert x (B b) f), (fid, Map.insert x (B b) f)]
...
constProp n facts =
  map_EN rewriteE n >>= return . toAGraph
  where
    rewriteE e@(Var v) =
...
map_EN :: (Expr -> Maybe Expr) -> Node e x -> Maybe (Node e x)
map_EN f (Label _ ) = Nothing
map_EN f (Assign x e@(Add e1 e2)) =
  case (f e1, f e2) of
    (Nothing, Nothing) -> f e
    (e1', e2') -> Just $ fmb e (f (fmb e1 e1') (fmb e2 e2'))
                    where fmb _ (Just e) = e
                          fmb e Nothing  = e
 ... more cases to rewrite expressions
\end{code}
\caption{The constant-propagation client} \figlabel{const-prop}
\end{figure}
...
depends on the open/closed-ness of the node's exit.}
...
\simon{John, You offered to reword this section, so I'm leaving it alone}
...
type ARF thing n f 
  = forall e x. f
             -> thing e x 
             -> FuelMonad (OutFact x f, Graph n e x, )
\end{code}
...
\end{code}
A ARF (short for ``analyse and rewrite forward'') takes an input fact and a @thing@,
and returns a correspondingly-shaped output fact and rewritten @Graph@.
...
\begin{itemize} 
\item From the supplied @ForwardTransfers@ and @ForwardRewrites@, produce a @ARF_Node@
...
\item From this @ARF_Node@, produce a @ARF_Block@ that transforms blocks.
\begin{code}
  arfBlock :: ARF_Node n f -> ARF_Block n f
\end{code}
 
\item From this @ARF_Block@, produce a @ARF_Graph@ that tranforms graphs.
\begin{code}
  arfGraph :: DataflowLattice f
           -> ARF_Block n f -> ARF_Graph n f
\end{code}
...
analyseAndRewriteFwd lat tf rw depth
 = anal_rw
 where 
  anal_rw, anal_only, arf_rec :: ARF_Graph n f
  anal_rw = arfGraph lat $ arfBlock $ arfNode tf rw arf_rec
  anal_tf = arfGraph lat $ arfBlock $ arfNodeNoRW tf
  arf_rec = case depth of
              RewriteShallow -> anal_tf
              RewriteDeep    -> anal_rw
\end{code}
}
This beautifully compact code does the following:
\begin{itemize}
\item The result, @anal_rw@ is built by composing
@arfGraph@, @arfBlock@, and @arfNode@.
\item The call to @arfNode@ passes the client-supplied transfer and rewrite
functions, and @arf_rec@ to analyse and further rewrite rewritten graphs.
\item When deep rewriting is used, the @arf_rec@ engine is simply a recursive
use of the main function, @anal_rw@. But if shallow rewriting is required, it instead
uses the shallow engine @anal_tf@. 
...
@arfGraph@ and @arfBlock@
to the shallow node engine, @arfNodeNoRW tf@.
