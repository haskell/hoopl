...
\ourlib\ recursively analyzes and rewrites~@g@ before moving on to the next node.
...
we must join the facts coming from those two places.  The fact flowing from
block @L1@ to @L3@ is $@x=3@ \land @y=4@ \land @z=False@$, while the fact
flowing from block @L2@ to @L3@ is
$@x=7@ \land @y=4@ \land @z=True@$.  
Both facts flow to @L3@
...
(\figref{lattice}). 
%%Such a value is part of the  @ForwardPass n f@ that is passed to
...
and a variable with an unknown value maps to @Nothing@ (i.e., it is not
...
in turn, @rewriteE@ function checks if the variable has a constant
...
Analysis computes a fixed point, and then it is up to the client of
the framework to use that fixed point for transformation.
Leaving transformation out of the framework greatly simplifies the
problem, but there is still a spectrum of designs in use.
We~aim not to be comprehensive but to mention two representatative
designs.
We~then move on to the two prior frameworks that support interleaved
analysis and transformation.
...
the framework.
The interface provided for the client is fairly complicated;
the purpose of the complexity appears to be to enable the client to
...
Soots interface for defining lattice and analysis functions is similar to our own,
although because Soot is implemented in an imperative style, 
additional functions are needed copy lattice elements.
...
by \citet{lerner-grove-chambers:2002}, who were the first to implement
interleaved analysis and transformation.
Their implementation is much like our early efforts:
it is a complicated mix of code that simultaneously manages the interleaving,
deep rewriting, and the fixed-point computation.
...
In previous work, we described a zipper-based representation of control-flow
...
\item
By using the power of GADTs, we are able to represent a flow-graph
node using a single type, instead of the triple of first, middle, and
last types used in our earlier representation.
This change in representation simplifies the interface significantly:
instead of providing three transfer functions per analysis---one for
each type of node---a client of \hoopl\ provides only one transfer
function per analysis, and similarly for rewrite functions.
\item
...
dataflow analysis and transformation.
While tackling this formidable challenge to our design skills,
we learned two kinds of lessons:
we learned some very specific things about representations and
algorithms for optimizing compilers,
and we were forcibly reminded of some very old lessons that are well
...
designing good abstractions is very challenging. 
\hoopl's data types and the functions over those types have been
through \emph{dozens} of revisions.
As~we were refining the design, we found it invaluable to operate in
two modes:
...
problem, and that we did not make a design decision that inadvertently
crippled an important part of the compiler outside \hoopl.
Operating in the second mode---as a standalone library---enabled us to
iterate furiously, trying out many more ideas than would ever have
...
We were forcibly reminded of what is already well known:
that interfaces are more important than implementations, and that data
is more important than code.
(Representation is \emph{still} the essence of programming.)
These priorities are reflected in this paper, where
we
have devoted nearly seven pages to \hoopl's API, and only a third of that
to its implementation.
We were also reminded that Haskell's type system (polymorphism, GADTs, higher 
order functions, type classes, and so on) is a remarkably effective
language for thinking about data and code---and that
Haskell lacks a system of interfaces (in ML parlance, signatures) that
would make it equally effective for thinking about APIs at a larger scale.
Still, as always, the types were a remarkable aid to writing the code:
...
examples of how types really helped us create our design:
\begin{itemize}
\item 
Reuse is enabled by representation-independence, which in Haskell is
...
\item
We are particularly proud of our use of GADTs to statically
track the open and closed shapes of nodes, blocks, and graphs.  
Shapes may
...
Finally, giving the \emph{same} shapes
...
we now have just one node type,
one transfer function, and one rewrite function.
But to make this implementation work, we \emph{must} have
the type-level function for
@Fact@ (\figref{api-types}), to express how incoming
and outoing facts depend on the shape of a node.
\end{itemize}

Dataflow optimization is usually sold as a way to improve imperative
programs by mutating control-flow graphs.
Such transformations are pretty far from the tree rewriting that works
so well at higher levels of compilers, 
but we learned that even so,
 writing a dataflow optimizer
in a pure functional language is a huge win.  We could not possibly
have conceived \ourlib{} in C++.
In~a functional language, not only do we know that
...
significnat improvements to the already-tricky Lerner/Grove/Chambers idea,
including
...
parts of the code that cannot possibly interact through shared mutable state.

An ancestor of \ourlib{} is in the Glasgow Haskell Compiler today, in version~6.12.
...
% omit delFromFactBase :: FactBase f -> [(Label,a)] -> FactBase f
...
%   the set of variables for which a reload is available.
%   We list the types of the functions that manipuate sets of available variables,
