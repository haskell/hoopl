...
loop contains a \texttt{break} statement}
\end{itemize}
...
@IfOpen e (Block n O C)@, 
\remark{Do people like the name @MaybeOpen@ or @MaybeO@ better than @IfOpen@?}
which is also defined in \figref{graph}:
the type @IfOpen O a@ is isomorphic to~@a@, while 
the type @IfOpen C a@ is isomorphic to~@()@.
\remark{Can we please consider changing the names of the value
constructors?
Possibilities include @IfOpen@/@IfNotOpen@ or @JustO@/@NothingC@.
}
\item 
...
@Body n@.
\item 
The exit sequence is dual to the entry sequence.
\end{itemize}
% 
...
From this table \remark{you see no table here}
