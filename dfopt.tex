...
\simon{My intention is that this section gives a quick introduction to why
interleaving analysis and rewrite is important.  I belive that we can 
leave the question of ``super-analyses'' to the next paper.}
...
As an example, here is a sequence of assignments separated by assertions.
...
\simon{Transformation (replacing conditional branches by unconditional ones) 
remains to be added}
...
\subsection{Example: constant propagation}

\simon{Here we give the transfer function for constant-prop}
...
\subsection{Using constant propagation to rewrite graphs}

\simon{Here we give a rewrite function for constant-prop}

\section{Reflections}

This is a place to collect thoughts about the API and the users-eye view.
I didn't want to clutter the initial tour with it.
