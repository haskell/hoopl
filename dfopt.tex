...
that preserves semantics.  \simon{The two sentences preceding this note say 
essentially the same thing.  Combine.}
...
$x=k$.
\simon{My taste: I don't think @TxRes@ is paying its way.  I'd replace it
with pair (a,@ChangeFlag@).}
...
(\figref{lattice}) and transfer functions of type @ForwardTransfers m l a@
(\figref{transfers}),
...
`availVarsLattice = DataflowLattice empty add
    where ^empty = UniverseMinus emptyVarSet
          ^add ^new ^old = let ^join = interAvail new old in
                        TxRes (if join `smallerAvail` old then SomeChange else NoChange) join
\end{code}
\B
& Lattice\\
\hline
...
These ideas
make it possible 
to think simple thoughts about classic code improvements.
% to think simple thoughts about classic code improvements.
\delendum{It would be good to give cross-refs to places in the paper that
