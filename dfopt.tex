...
\begin{abstract}
\remark{Needs a complete rewrite}\simon{It's not so bad, is it?}
...
An optimising compiler for an imperative language includes many
analyses, and transformations driven by the results of those analyses.
Although these analyses and transformations are known by many names 
--- constant propagation, live variable analysis, inlining, sinking of loads,
and so on --- many of them can be regarded as particular cases of
a single general framework: \emph{dataflow analysis and rewriting 
of control-flow graphs} \cite{this and that}.  Dataflow analysis
is as old as the hills, but a relatively recent, seminal
paper by Lerner, Grove and Chambers goes further, 
describing a powerful but subtle way to
\emph{interleave} analysis and transformation so that each 
piggy-backs on the other.

Since they share a common intellectual framework, and that framework is
...
situations.
Attractive, but difficult.  Some such frameworks exist, as we discuss 
in \secref{related}, but they have complex APIs and implementations,
and none implements the Lerner/Grove/Chambers technique.

In this paper we present \ourlib{} (short for ``higher-order
optimisation library''), a new Haskell library for dataflow analysis and
optimisation.  It has the following distinctive characteristics:

\begin{itemize}
\item It is purely functional.  We argue at length in an earlier 
paper \cite{norman and john} that, perhaps surprisingly, code that
manipulates control flow graphs is easier to write, and far easier
to get right, when written in a purely-functional style.  This 
point comes out particularly clearly when analysis and rewriting
are interleaved, so that rewriting must be done without knowing whether
the result of the rewrite will be retained or discarded 
(Sections \ref{sec:overview} and \ref{sec:fixpoints}).

\item It is polymorphic. Just as a list library is
polymorphic in the list elements, so \ourlib{} is polymorphic, both in
...
a transfer function that transform assertions across a node, 
and a rewrite function that uses a computed asserion to 
justify rewriting a node.
\ourlib\ ``lifts'' these node-level functions to work over
control-flow graphs, setting up and solving recursion equations,
and interleaving rewriting and analyis.

\item Analyses and transformations built on \ourlib\ 
are small, simple, and easy to get right.
Moreover, \ourlib\ helps you write correct optimisations:
...
\item \ourlib{} implements a rather subtle algorithm.  Our (three) 
previous attempts to implement this algorithm were complicated and
hard to understand. In contrast, in \ourlib{} we have found a new way
to structure the implementation so that various tricky pieces 
(fixpoints, interleaved rewriting and transformation, recursive
rewriting, optimisation fuel, and so on) are each located in just 
one place.  Indeed, the result is sufficiently elegant that 
this paper treats the implementation as an object of interest in its
own right (\secref{engine}).
\end{itemize}
A working prototype of \ourlib{} is available for download 
at \simon{add url}.  It is no toy: an ancestor of this library is
embodied in the Glasgow Haskell Compiler, where it optimises the
imperative @C--@ code in GHC's back end.  The new design is far
nicer, and will be in GHC shortly.

The API for \ourlib{} seems quite natural, but requires
...
transformation?  Yes: constant propagation can rewrite it to @z:=7>5@.
Now, can we do another transformation?  Yes: constant folding can rewrite it
to @z:=True@.
And so the process continues to the end of the block, where we
can transform the conditional branch to an unconditional one, @goto L1@.
...
To summarise, suppose E is a node, block, or graph:
\begin{itemize}
...
\ourlib{} composes blocks into graphs, which are also defined by
\ourlib\ in  \figref{graph}.
...
the graph as a finite map from @BlockId@ to \mbox{@Block@ @n@ @C@ @C@}.  
...
constructor, and it contains only a @BlockId@.\remark{Why a @BlockId@
instead of nothing?}
...
%at block $l$. 
We need the @BlockId@ $l$, because the @PGraph@ 
...
\end{itemize}
Partial graphs compose nicely.  
For example, like blocks, two graphs may be concatenated exactly when
they are both open at the point of concatenation:
\par{\small
\begin{code}
...
analyseAndRewriteFwd
  ...
  -> Graph n                -- Input graph
  -> FuelMonad (Graph n,    -- Result graph
                FactBase f) -- ...and its facts

type FactBase f = BlockMap f
\end{code}
Roughly speaking, given a lattice, a transfer function, and a rewrite
function, @analyseAndRewriteFwd@ can transform a @Graph@ into an
optimised @Graph@.  One thing that is immediately obvious from this
type is that it is polymorphic in the type of nodes @n@ and facts @f@;
these are entirely under the control of the client.

As well as taking and returning a @Graph@, the function also takes
some input facts (the @FactBase@) and produces output facts. A
@FactBase@ is simply a finite mapping from @BlockId@ to facts.  The
output @FactBase@ maps each @BlockId@ in the @Graph@ to its fact; if
the @BlockId@ is not in the domain of the @FactBase@ its fact is the
bottom of the lattice.  Similarly the input @FactBase@ supplies any
facts that hold on entry to the @Graph@.  For example, in our constant
propagation example, if the @Graph@ represents the body of a procedure
with parameters $x,y,z$ we would map the entry @BlockId@ to a fact
...
type `BackwardRewrite n f 
  = forall e x. n e x -> TailFactB x f
             -> Maybe (AGraph n e x)
...
\subsection{The {\tt RewritingDepth} flag}
...
regHasConst (Branch l)          f = [(l, f)]
regHasConst (Cond (Reg x) tid fid) f
  = [(tid, Map.insert x (B True)  f),
     (fid, Map.insert x (B False) f)]
regHasConst (Cond _ tid fid)    f = [(tid, f), (fid, f)]
...
We have written two @ForwardRewrites@ functions 
because they are sometimes independely useful.  But in this case we
want to apply them successively, first @constProp@ and then @simplify@.
So we compose them together with a simple combinator, @combineFRW@, 
and pass the combined rewrite to @analyseAndRewriteFwd@ (see \figref{const-prop}).

Note that @simplify@ does not need to check whether a register holds a constant
value, because it can rely @constProp@ to have replaced the register by the constant;
indeed @simplify@ does not consult the incoming fact at all, and hence is
polymorphic in @f@.

We can invoke the analysis by calling  @analyseAndRewriteFwd@.
with the lattice, the transfer function, the combined rewrite functions,
an empty @FactBase@, and the input graph.
Because constant-propagation and constant-folding are mutually beneficial,
we use deep rewriting to apply both transformations
recursively.  \simon{I find it hard to see how deep rewriting could be beneficial here.}
...
tpye signature is in \secref{using-hoopl}.
...
Why do we return @RG@ not @PGraph@?  
We return a rewritten graph @RG@ rather than a @PGraph@ 
for two reasons:
\begin{itemize}
\item The client is often interested not only in the facts flowing
out of the graph (which are returned as the @TailFactF@), 
...
\item A @PGraph@ has deliberately restrictive invariants; for example,
a @PMany@ with a @Tail@ is always open at exit (\figref{graph}).  It turns
...
\item From this @ARF_Block@, produce an @ARF_Blocks@ that 
transforms a @Graph@:
\begin{code}
'arfBlocks :: forall n f. DataflowLattice f 
           -> ARF_Node n f 
           -> FactBase f -> Graph n 
           -> FuelMonad (FactBase f, (Graph n, FactBase f))
\end{code}

\item From this @ARF_Blocks@, produce a @ARF_Graph@ that tranforms graphs (\secref{arf-graph}):
\begin{code}
...
\subsection{From nodes to blocks} \seclabel{arf-node}
...
\cite{optimizing-soot-hendren-2000}.
...
\john{Is it worth mentioning that frameworks without interleaving don't seem to need
 any explicit support for rewriting?}

The CIL toolkit \cite{cil-necula-2002}\john{Again, no good citation for same reason}
...
which of course executes interleaved analyses and transformations.
...
the implementation in Whirlwind is splits into two separate phases.
...
for a phase distinction; consequently, we do not even need
to~represent potential transformations.

Our implementation also provides additional support for the
optimization-writer:
\begin{itemize}
\item Using static type guarantees, we~rule out a whole
  class of possible bugs: transformations that produced malformed
  control-flow graphs.
\item Using Whalley's \cite{whalley:isolation} debugging technique,
