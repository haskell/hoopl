...
(a~backward analysis calls @`zdfSolveBwd@, which has a similar type):
\begin{code}
 `zdfSolveFwd 
...
@`LocalVar@) or global 
(@`CmmGlobal `GlobalVar@). %%% ; a @`LocalVar@ is always local.
...
which nodes.
Before explaining how to use dataflow facts and rewrite functions to
transform graphs, we therefore
explain the types and representation of
...
\ourlib\ transforms the graph structures just described by composing
the transfer functions described in \secref{tffuns} with
\emph{rewrite functions};
rewrite functions have the types 
...
control-flow graph (a~backward transformation uses @`zdfRewriteBwd@,
...
!insertLateReloads.2!               avail_vars_transfer availRewrites (fact_bot availVarsLattice) g
\end{code}
...
performed.
This phase is called the \emph{actualizer}; it performs the
rewrites for real.
\end{itemize}
...
And unless the client code specifies shallow rewriting,
\ifcutting
...
!solve.first.sig!         `iter_first :: BlockId -> FactKont a b -> Kont     a b
         iter_mid   :: m       -> FactKont a b -> FactKont a b
...
points;
defining one analysis function, including speculative rewriting, for
each type of graph node; 
and
using continuation-passing style to stitch these functions together
in a way that interleaves analysis with rewriting.
...
Analysis begins with an environment mapping all labels to bottom facts.
...
Each iterator interleaves analysis and transformation
     \cite{lerner-grove-chambers:2002}. 
     At each node, the iterator passes the node and
     the incoming dataflow facts to a rewriting function.
     If the node is rewritten, the iterator continues with the same dataflow facts
...
Each~iterator returns a set of dataflow facts, leaving the graph
unchanged. 
An~\emph{actualizer} takes a set of facts and a graph, and in a single
pass uses the rewrite functions to create a new graph.
The~actualizer also uses the transfer functions to materialize facts
on edges within basic blocks.

Function @zdfSolveFwd@ is implemented by calling the forward iterator
with the transfer functions given and with degenerate rewrite
functions that always return @Nothing@.
\remark{Lies!  It's done with fuel!}
Function @zdfRewriteFwd@ is implemented by calling first the forward
iterator, then the forward actualizer.
Functions @zdfSolveBwd@ and @zdfRewriteBwd@ are implemented similarly.
...
At each basic block, function @iter_block@ (type on \lineref{solve.block.sig})
computes the dataflow solution for the block.
Function @iter_block@ is the composition of
...
the implementation of @iter_block@.
\footnote{Similar
...
Because the iterator functions for nodes
% @iter_first@, @iter_mid@, and @iter_last@
resemble one another, we show only
% Ultimately, \emph{all} iterator functions
% are composed of applications of @iter_first@,
...
on~\lineref{solve.OO}
by a recursive call to @iter_OO@ (not shown).
...
by @iter_OO@ as it iterates to a fixed point, but @subAnalysis@
does account for 
...
\figref{actualizer-excerpts} is much like function @iter_mid@ on 
\linerangeref{solve.mid.1}{solve.mid.*} of \figref{iterator-excerpts}.
...
Similar functions apply to graphs of other shapes.
