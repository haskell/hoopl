...
as @BUnit@ lifted a node to a block.  
...
but by a whole graph.  For example, perhaps we want to replace a high-level operation
with a set of conditional branches, which entails introducing new blocks.
...
The rewrite function should, of course, be \emph{sound}:
if it replaces a node @n@ by a replacmement graph
