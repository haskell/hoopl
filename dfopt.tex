...
\begin{code}
 zdfSolveFwd 
  ...
  -> DFMonad (ForwardFixedPoint m l a)
\end{code}
...
zdfSolveFwd name lattice transfers in_fact g = 
    runDFM lattice $ ffp () $ fwd_pure_anal name emptyBlockEnv transfers in_fact g
fwd_pure_anal name env transfers in_fact g =
    forward_sol (\_ _ -> Nothing) undefined name env transfers undefined in_fact g undefined >>= 
    return . fst
ffp :: b -> DFM a (Maybe a) -> DFM a (ForwardFixedPoint m l a b)
\end{numberedcode}
...
\simon{There is a confusion of terminology.  In this section 
...
\simon{(I wish I could put paragraphs in the middle of comments!
...
rewriter, and simply discard teh returned graph.  Correct?
...
\simon{New remark.  I found myself deep in fuel administration when looking 
