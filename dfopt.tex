...
\begin{verbatim}
  ...
  z = x + y;
\end{verbatim}
\simon{Could we add $x=7,y=8,z=15$ as a final assertion?}
...
representation.
(An~optimizer also works with \emph{subgraphs}, in which one or two
...
!solve.mid.Just!         Just (fuel, g) ->
           do { (a, fuel) <- subAnalysis $
                   case depth of
                     RewriteDeep -> solve_OO' g return2 in' fuel
                     RewriteShallow -> do { a <- anal_f_OO g in'
                                          ; return (a, fuel) }
!solve.mid.*!              ; k a fuel }
\end{numberedcode}
%%  
...
%%  !setFact.g.entry!           solve in_fact g fuel = do { setFact (g_entry g) in_fact
%%                                       ; fuel <- run "forward" name set_successor_facts blocks fuel
%%                                       ; b <- getExitFact                                          
%%  !solve.*!                                     ; return (b, fuel) }                                        
%%  !solve.blocks!             where blocks = postorder_dfs g
...
This section shows that the complexity is {manageable}.

\simon{This section is better, although I have not read every line.
But it still doesn't answer the questions I asked earlier:
What new insights
...
\secref{graph.intro} oversimplifies the representation of graphs.
It is true that a graph is a collection of basic blocks, 
and that a typical basic block has one first node followed by zero or
more middle nodes followed by one last node.
But~in addition, a graph may contain two special, incomplete basic
blocks:
\begin{itemize}
\item
...
\end{itemize}
The representation of both graphs and subgraphs is therefore a triple:
...
graphs provide a constant-type concatentation operator @<*>@, and for
example if $g_1$'s shape is $e$/open and $g_2$'s shape is open/$x$,
then $g_1 \mathbin{\mbox{@<*>@}} g_2$ is well defined and has shape
...
\item
A middle node must be rewritten to an open/open graph, that is, a
single-entry, single-exit subgraph.
\item
A last node must be rewritten to an open/closed graph, that is, a
graph with an entry sequence but no exit sequence: all control-flow
...
is out of fuel. 
We~do so using parameter @with_fuel@.
\item
An analysis of a subgraph starts with known facts @start_facts@, not
bottom facts; they are passed as a parameter
(\lineref{forward.sol.args}) and
 set by @setAllFacts@ on \lineref{forward.sol.setAllFacts}.
\item
...
\begin{itemize}
\item
Type @FactKont a b@ gives the context following a first node or middle
...
Type @FactKont a b@ also gives the context following the analysis of
any replacement graph that is open at the exit.
\item
Type @LOFsKont a b@ gives the context following a last node.
...
@LastOuts a@.
\item
Type @FuelKont a b@ gives the context \emph{before} a first node (or a
basic block).
The forward analysis of a basic block does \emph{not} start with any
...
\end{itemize}
\Linerangeref{solve.first.sig}{solve.last.sig} give the types of
...
But once we have analyzed a block, we want to update the facts stored in
the inner dataflow monad---iterative dataflow analysis \emph{means}
repeatedly updating stored facts until they reach a fixed point.
Stored facts are updated by function @set_last@ on
\linerangeref{set.last.sig}{set.last.*} of 
\figref{solver-excerpts}, which simply calls @setFact@ on each fact
...
The~type says that @solve_block@ is given a block and an amount of
fuel;
it runs for its side effects on the inner dataflow monad, and it
returns the remaining fuel.

Space limitations prevent us from presenting the rest of the solver
in detail.
To~solve a list of blocks, we iterate @solve_block@ until reaching a
fixed point.
We~can then write solvers for subgraphs which are open/open,
open/closed, or closed/open, and finally @solve_Ox@, which solves an
entire graph~@g@.
Graph~@g@ is open at the entry and may be open or closed at
the exit: if open, it produces an exit fact; if closed, it does not.
The solver for an entire graph therefore has side effects on the inner
dataflow monad, and it returns a pair containing a possible exit fact
...
\begin{numberedcode}
    ...
    rewrite start in_fact g fuel =
      let blocks = postorder_dfs g
!call.forward.sol!      in do { forward_sol squash depth name start transfers rewrites in_fact g fuel
...
We do feel compelled to give details of one solver function:
all other functions are composed of applications of @solve_first@,
@solve_mid@, and @solve_last@, and on
\linerangeref{solve.mid.1}{solve.mid.*} of \figref{solver-excerpts},
we show @solve_mid@ in full.
On~\lineref{solve.mid.case}, a rewrite function is presented with an
...
produces @Nothing@, and the continuation~@k@ is given the output fact
(computed by the transfer function on \lineref{solve.mid.Nothing}) and
the supply of fuel.
...
\ifpagetuning\enlargethispage{-\baselineskip}\fi % reduce ugly gaps
...
(\figref{rewriter-excerpts}, \lineref{call.forward.sol}).
...
a replacement graph~@g@ (\linerangeref{rewrite.rewrite.1}{rewrite.rewrite.*}).
...
spliced onto the head of the block
\cite{ramsey-dias:applicative-flow-graph}, producing a new head~@h@
and a set 
...
forward_sol squash depth name start_facts transfers rewrites = fixed_point
 where
   ...
   solve finish in_fact g@(Graph eid blockenv) fuel =
       do { setFact eid in_fact
          ; fuel <- run "forward" name set_successor_facts blocks fuel
          ; b <- finish
          ; return (b, fuel)
          }
   ...
   either_last rewrites in' (LastExit) = Nothing
...
%%                      ; return (g_blocks g' `plusBlockEnv` rewritten, fuel) }
%%     either_last rewrites in' (LastExit) = in'
%%     either_last rewrites in' (LastOther l) = fr_last rewrites l in'
%% \end{numberedcode}
...
forward_rew squash depth xstart_facts name transfers rewrites in_factx gx fuelx = fixed_pt_and_fuel
  where
    ...
    rewrite start finish in_fact g fuel =
      let Graph entry _ = g
          blocks = G.postorder_dfs g
      in do { solve depth name start transfers rewrites in_fact g fuel
            ; (rewritten, fuel) <- rewrite_blocks EntryBlock blocks emptyBlockEnv fuel
            ; a <- finish
            ; return (a, Graph entry rewritten, fuel)
            }
    don't_rewrite facts finish in_fact g fuel =
        do  { solve depth name facts transfers rewrites in_fact g fuel
            ; a <- finish
    ...
    inner_rew f i g fu = getAllFacts >>= \facts -> inner_rew' facts f i g fu
        where inner_rew' = case depth of RewriteShallow -> don't_rewrite
                                         RewriteDeep    -> rewrite
    rewrite_blocks :: BlockPos -> [Block m l] -> (BlockEnv (Block m l))
                   -> Fuel -> DFM a (BlockEnv (Block m l), Fuel)
    rewrite_blocks _ [] rewritten fuel = return (rewritten, fuel)
    rewrite_blocks pos (G.Block id t : bs) rewritten fuel =
      do let h = ZFirst id off
         a <- getFact id
         case squash fuel $ fr_first rewrites id a of
           Nothing -> do { (rewritten, fuel) <-
                              rew_tail h (ft_first_out transfers id a) t rewritten fuel
                         ; rewrite_blocks NonEntryBlock bs rewritten fuel }
           Just g  -> do { markGraphRewritten
                         ; (outfact, g, fuel) <- inner_rew getExitFact a g fuel
                         ; let (Graph _ blocks, h) = splice_head h g
                         ; (rewritten, fuel) <-
    ...
    rew_tail head in' (G.ZTail m t) rewritten fuel =
      case squash fuel $ fr_middle rewrites m in' of
        ...
        Just g -> do { markGraphRewritten
                     ; (a, g, fuel) <- inner_rew getExitFact in' g fuel
                     ; let (Graph _ blocks, h) = G.splice_head head g
                     ; rew_tail h a t (blocks `plusBlockEnv` rewritten) fuel }
    rew_tail h in' (G.ZLast l) rewritten fuel = 
      case squash fuel $ either_last rewrites in' l of
        Nothing -> do return (insertBlock (zipht h (G.ZLast l)) rewritten, fuel)
        Just g -> do { markGraphRewritten
                     ; ((), g, fuel) <- inner_rew (return ()) in' g fuel
                     ; let g' = G.splice_head_only h g
                     ; return (G.g_blocks g' `plusBlockEnv` rewritten, fuel) }
    either_last rewrites in' (LastExit) = Nothing
    either_last rewrites in' (LastOther l) = fr_last rewrites l in'
\end{numberedcode}
\caption{The forward rewriter}
