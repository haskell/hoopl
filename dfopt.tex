...
Nevertheless, \emph{those rewrites
...
of @x==10@.  Again the first iteration will begin with the ``fact'' \{x=0\}.
...
\item Begin each iteration with the orignal, virgin graph.
\end{itemize}
...
\begin{code}
  gftBlock :: ARF_Node n f -> ARF_Block n f
\end{code}
...
analyseAndRewriteFwd lat tf rw depth
  ...
  where 
    ...
    anal_rewrite = arfGraph lat $ arfBlock $ arfNode tf rw arf_rec

    anal_only    = arfGraph lat $ arfBlock $ arfNodeTransfer tf

    arf_rec = case depth of
                RewriteShallow -> anal_only
                RewriteDeep    -> anal_rewrite

arfNodeTransfer :: ForwardTranfsers n f
                -> ARF_Node n f
arfNodeTransfers tf f n = return (f, nodeToGraph n)
\end{code}
}
This beautifully compact code does the following:
\begin{itemize}
\item The result, @anal_rewrite@ is built by applying @arfGraph@ and @arfBlock@ to
the node-rewriting engine, @arf_n@.
\item The node-rewriting engine instructs @arfNode@
to use @arf_rec@ for recursive applications to rewritten graphs.
\item When deep rewriting is used, the @arf_rec@ engine is simply a recursive
use of the main function, @anal_rewrite@. But if shallow rewriting is required, it instead
uses the shallow engine @anal_only@. 
...
to the shallow node engine, @arfNodeTransfer tf@.
\item Finally, the shallow node engine, which merely performs dataflow transfers on a single
node, with no rewriting, is built with @arfNodeTransfer@.
\end{itemize}
