...
must write a function that is presented with a flow-graph node and
with the dataflow facts on the edges coming
into that node.
...
\subsection{Transfer functions and representations of lattices}

A~dataflow analysis is presented with dataflow facts on edges coming
into each node, and it computes the dataflow facts on outgoing edges;
in other words, an analysis implements \emph{transfer functions}.
...
(call it $f_2$), with the lattice join $f \join f_2$.
The dataflow engine iterates over the blocks repeatedly until
$f \join f_2 = f_2$ at every block.
...
We use a nonstandard representation of lattice operations,
...
$@TxRes NoChange@\; (f \join f_2)$ if $f \join f_2 = f_2$
and is equal to
$@TxRes SomeChange@\; (f \join f_2)$ otherwise.
...
is equal to the right argument.
%
The @fact_name@ element of a lattice record is used for debugging.
...
{The type of @zdfSolveFwd@ contains pedagogical lies;
...
%\subsection{Spilling variables at calls}
