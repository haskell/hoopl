...
\preprintfooter{\input{timestamp}}
...
\remark{Try to abuse {\LaTeX} to pull those 3 lines to page 1}
...
Our big claim is that our library
...
Answering this question requires a POPL paper
\remark{It would be nice to have some insight into why the soundness
of the rewrite functions wrt the incoming dataflow facts is sufficient
to make the whole algorithm sound.}
\cite{lerner-grove-chambers:2002}, but we can at least give some
intuition about termination.
\begin{itemize} 
\item 
As noted above, rewrite functions may not return graphs that contain
nodes which are rewritten indefinitely.
Alternatively, 
by using the @RewritingDepth@ parameter,
@zdfRewriteFwd@ can be told not to rewrite graphs returned from a
rewrite function. 
\item
...
!forward.sol.args!    forward_sol squash depth name start_facts transfers rewrites = ...
...
Above, in sections \ref{sec:example:transforms}
through~\ref{sec:rewrites},
we show how to use our library to create analyses and transformations.
...
the implementation of dataflow engine is not simple.
...
optimization selectively in order to isolate faults  (\secref{vpoiso}).
...
\remark{Perhaps this \P\ could be cut drastically, and we can refer
back to the specification in \secref{dfengine-spec}?}
...
@zdfSolveFwd@.
Because it is also used to solve subgraphs and to help the rewriter, it
...
takes an input fact, a graph, and fuel supply, and it
returns a pair containing the exit fact and the
remaining fuel supply.
It~also has a side effect on the state stored in the dataflow monad:
...
@set_successor_facts@ each block in the graph, until the facts
stored in the dataflow monad stop changing.
