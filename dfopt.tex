...
This purpose of this section is to show that the complexity is \emph{manageable}.
\remark{What's new? Polymorphism, purity, type classes---purity makes
it easier to expunge bugs, especailly when managing state for things
like sub-analyses.}

\emph{[Our implementation improves on our 2005 work in two
ways:\remark{Not sure here is the palce for this}
\begin{itemize}
\item
The control-flow graph and dataflow engine are polymorphic in the
types of nodes.
This small change makes it much easier to separate concerns;
in particular, it forces us to make explicit \emph{exactly} what the
dataflow engine must know about flow-graph nodes.
(The answer is, it must be able to enumerate the successors of a last
...
We show excerpts the forward solver and rewriter in
...
The interesting case occurs on \linerangeref{solver.rewrite.1}{solver.rewrite.*},
...
As~\linerangeref{solver.rewrite.1}{solver.rewrite.*} show, the new
...
(\linerangeref{solver.rewrite.1}{solver.rewrite.*}) and not 
...
\citet{ramsey-dias:applicative-flow-graph} describe an
...
!rewrite.squash!      case squash fuel $ either_last rewrites in' l of
