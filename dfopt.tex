...
{A variable $\tempof x = \slotof x$ at a label~@L@ if and
only if $\tempof x = \slotof x$ on every control-flow edge coming into~@L@.}
...
It is optimistic to assume that MUMBLE,\remark{fix me}
but this optimistic assumption is what enables us to improve the
code---and if the assumption is not justified, it will be corrected by
...
brief comments to the type signatures.   \textbf{NR}: I~second this proposal.}
...
After such an assignment $\tempof x = \slotof x$,
and the last definition of $tempof x$ is a reload,
...
After such a node, $\tempOf x = \slotOf x$,
but there is no guarantee that $\tempOf x$ was defined by a reload instruction,
