...
\simon{Doesn't the rewrite have to be have the following property:
...
\simon{Version 2, after further rumination.  Let's define
$rt(f,s) = \mathit{transform}(f, \mathit{rewrite}(f,s))$. Then $rt$ should
...
performed.
This phase is called the \emph{actualizer}.
\end{itemize}
...
The iterators perform iterative dataflow analyses.
...
and also to implement the iterator phase of @zdfRewriteFwd@.
Because @fwd_iter@ is used to solve replacement graphs as well as
full graphs, it
takes more parameters than @zdfSolveFwd@, and it is more polymorphic
(\figref{iterator-excerpts}, \linerangeref{forward.sol.sig}{forward.sol.args}).
\begin{itemize}
\item
%In the iterator phase described in \secref{iterator-phase},
...
the analysis
\emph{simulates} the effects of a fuel limit on the actualizer, but it does
...
% Function @fwd_iter@ is composed of many parts, all written in
...
% \figref{iterator-excerpts} give the types of
% iterators for first, middle, and last nodes.
...
% \figref{iterator-excerpts}, which calls @setFact@ on each fact
% flowing out of a block.
% By applying the composition
% of @solve_first@, @solve_mid@, and @solve_last@ to @set_last@, we get
...
% \emph{simulates} the effects of a fuel limit on the actualizer, but does
% not actually consume any fuel.
% Using @solve_blocks@, we have written iterators for subgraphs which are open/open,
...
@fwd_iter@ using the @with_fuel@ function
@\ _ _ -> Nothing@, 
...
%%  (\lineref{iterator.rewrite.1}), which means it has fresh 
...
%%  As~\linerangeref{iterator.rewrite.1}{iterator.tail.*} show, the new
...
forward iterator, but because the actualizer passes the rewritten graph as
...
the iterator has set the state of the dataflow monad to a fixed
...
\figref{actualizer-excerpts} is much like function @solve_mid@ on 
\linerangeref{solve.mid.1}{solve.mid.*} of \figref{iterator-excerpts}.
...
It first calls the iterator to set the dataflow facts to a fixed
