...
\newcommand\ourlib{Hoopl}  % also Hoppy, Hoopt, Hoopl
...
``optimization.''
But writing these optimizations is complicated
because the work is typically presented
...
Further complications arise if you want to implement a ``super-analysis,''
which can improve the code further by
combining symbiotic analyses in a monolithic pass.
...
about programs is that in dataflow optimization, assertions are approximated,
and all assertions are computed automatically by setting up recursion
equations and solving for a fixed point.
...
even in a purely functional setting.
Our contributions are as follows:
\begin{itemize}
\item
\ourlib\ defines a simple interface for implementing analyses and transformations:
you provide the representation for assertions and the functions that transform assertions,
and \ourlib\ sets up and solves the necessary recursion equations.
...
\ourlib\ can perform a super-analysis by \emph{interleaving}
simple analyses and transformations.
The interleaving algorithm is tricky to implement correctly,
but by using
generalized algebraic data types
and continuation-passing style,
our new implementation expresses the algorithm with a clarity and a degree of
...
\item
\ourlib\ helps the user write correct optimizations both by
statically ruling out transformations that violate invariants
of the control-flow graph,
and 
by identifying the first transformation that introduces a fault
in a test program (using an adaptation of Whalley's \citeyearpar{whalley:isolation} algorithm).
\item
\ourlib's polymorphic, higher-order design makes it possible to reuse
the library with many low-level codes.
\hoopl\ is designed to help optimize imperative procedures
(after inlining)\john{Why the ``after inlining'' restriction?}
with arbitrary control flow,
including intermediate languages and machine languages.
...
Our examples use our low-level compiler-target
language,~{\PAL}
...
to the part of \ourlib\ we call the \emph{dataflow engine}.
The dataflow engine defines
\emph{solver functions} and \emph{rewrite functions}.
The solver functions 
use a forward or backward \emph{analysis} to compute 
...
The dataflow engine uses the lattice's join operation in a stylized way.
...
\B
&
\def\baselinestretch{0.8}Available-variables analysis
%\B
\\
%\hline
...
Type @CmmVar@ represents a variable which is local (@CmmLocal@) or global
...
constructors include @CmmLoad@ (a~value from memory)
\label{haskell.def.CmmLoad}% automated definition
and @CmmVar@ (the value of a variable), among others.
...
each variable~$x$ is related to a stack slot~\slotof x that is used to
save the value of~$x$. 
(GHC~represents 
...
\label{haskell.def.variableOfSlot}% automated definition
...
\john{Check: does the reader already have enough intuition for this paragraph?}
...
!ZKont!type ZKont ex a b = ZMaybe ex a -> DFM a b

!forward.sol.sig!forward_sol :: forall m l e x a . HavingSuccessors l => (forall b . Maybe b -> DFM a (Maybe b))
...
!forward.sol.setAllFacts!     do { setAllFacts start_facts ; solve_ex g in_fact }
   where 
!solve.ex.sig!     solve_ex    :: GF m l e x -> ZKont e a (ZMaybe x a)
...
!solve.block.sig!     solve_block :: Block m l -> Kont a ()
...
!solve.OO!                                              RewriteDeep    -> solve_OO g return in'
!solve.rewrite.shallow.1!                                              RewriteShallow -> anal_f_OO g in'
!solve.mid.*!                       ; k a }
\end{numberedcode}
\caption{Excerpts from the forward solver}
\figlabel{solver-excerpts}
% !solve.block!     `solve_block (Block id tail) = solve_first id $ solve_tail tail $ set_last
...
\label{haskell.def.solve:unex}% automated definition
...
\label{haskell.def.ZKont}% automated definition
...
\label{haskell.firstuse.ZKont}% automated use
...
\label{haskell.firstuse.solve:unex}% automated use
...
the dataflow engine.
The dataflow engine is less simple than the title of this paper might
lead you to expect.
...
but the totality of the dataflow engine is complex.
...
Rewriting is similar to solving, with two differences:
\begin{itemize}
\item Rewriting begins with an interleaved analysis, according to the algorithm of
     ...
     If the node is rewritten, then the analysis continues with the same dataflow facts
...
\item After the analysis terminates, one more pass visits the blocks,
...
both return results in the fuel monad.
But to simplify the implementation of the dataflow engine,
...
dataflow fact and @b@ is the type of the value computed by the monadic action.
The dataflow monad also maintains the supply of optimization fuel
and the supply of fresh labels.
...
To prevent a node from being rewritten,
either because
we are implementing a pure analysis, or because the optimizer
is out of fuel, we  use parameter @with_fuel@
\label{haskell.def.with:unfuel}% automated definition
(\lineref{forward.sol.args}, called on \lineref{solve.mid.case}),
which can prevent rewriting by 
...
helped us eliminate dynamic tests of the shapes of subgraphs.
\end{itemize}
...
!rew.first!      rew_first :: BlockId -> GraphFactKont m l e O a b -> GraphKont     m l e C a b
      rew_mid   :: m       -> GraphFactKont m l e O a b -> GraphFactKont m l e O a b
!rew.last!      rew_last  :: l       -> GraphKont     m l e C a b -> GraphFactKont m l e O a b
...
!sar.OO!      sar_OO :: GF m l O O -> GraphFactKont m l O O a b -> FactKont a b
\end{numberedcode}
...
% Function @zdfSolveFwd@ calls @forward_sol@:
% \begin{smallcode}
...
The types of the
continuations are shown on \linerangeref{FactKont}{ZKont} of
\figref{solver-excerpts}. 
\begin{itemize}
\item
...
fact and the remaining fuel and produces a computation in the
...
\item
Type @ZKont e a b@ describes a context \emph{before} the graph
being analyzed, where @e@~tells whether the graph is open at the entry.
\end{itemize}
...
function @solve_block@, whose type is given on \lineref{solve.block.sig}.
Function @solve_block@ is given a block and
...
Finally, @solve_ex@ (type on \lineref{solve.ex.sig}, implementation not
shown), which provides the external interface, 
solves the full graph~@g@.
...
the fuel consumed in the final iteration.\john{The final iteration of what? This sentence suggests that subAnalysis recognizes the final iteration of whatever it's running; surely it doesn't.}
\item
