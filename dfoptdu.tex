...
\john{We should agree on a consistent hyphenation of control-flow graph / control flow graph,
  which differs throughout this paper. I don't know if it's a British-vs-American
  English thing, but we should settle on something. Frankly, I don't
  care about this case, since the meaning is clear,
  but I know Norman's a stickler for this, and the American version would be hyphenated.
  Is the British hyphenation rule different?}

\norman{The British rule is exactly the same.  If Simon does not own a
  copy of Fowler, his eldest unmarried daughter does.}
...
\john{This example is really a combination of constant propagation and
 constant folding, as noted later in the paper; I think we should make that clear here.}
...
transformation?
Yes!  We can replace it with @x=7@.\john{That's constant-folding, not constant-prop.}
...
\label{haskell.firstuse.>}% automated use
transformation?  Yes: replace it with @z=True@.
\label{haskell.firstuse.True}% automated use
And so the process continues. Notice that in the last node of the block we can transform
a conditional branch to an unconditional one.
...
fact \{@x=7@, @z=True@\}.  But the subsequent transformation must 
...
examples.
\john{I think this last sentence should use the reference not to point out that this
approach makes things simpler, but to assert that: by using interleaving, not only
can you write simpler, compositional analyses, but you can get
better results than by running the analyses in sequences (and cite here).}
\john{Also, I think this example only does half the job: we should show the reader
how the example becomes simpler when you have interleaving:
const-prop uses {x==7} to produce z = 7 > 5,
then constant-folding doesn't need any facts to produce z = True.
Furthermore, even the implementation of the analysis requires absolutely no smarts:
no special case required for x := y where y already holds a constant value.
...
so \ourlib{} knows nothing about their representation (\secref{nodes}).
...
\emph{nodes}, represented by a data type defined by the client.
...
This restriction is quite standard \cite{appel:modern}.
It dramatically simplifies control-flow graph analyses
and transformations,
and it costs nothing in practice:
a late-stage code-layout pass can readily reconstruct the efficient code.
...
entry and exit respectively.  It is defined in GADT-style syntax
\cite{peyton-jones:generalized-algebraic},
by giving the types of each of its constructors.  For example, constructor @Label@
...
\label{haskell.firstuse.BlockGraph}% automated use
\label{haskell.firstuse.BlockMap}% automated use
\label{haskell.firstuse.a}% automated use
\label{haskell.firstuse.is}% automated use
\label{haskell.firstuse.finite}% automated use
\label{haskell.firstuse.map}% automated use
\label{haskell.firstuse.from}% automated use
\label{haskell.firstuse.to}% automated use
\label{haskell.firstuse.Graph}% automated use
\label{haskell.firstuse.GNil}% automated use
...
\label{haskell.firstuse.NoTail}% automated use
\end{figure}

So much for nodes.  The types for blocks and graphs are defined by \ourlib{}
itself, with definitions given in \figref{graph}.
A @Block@ 
is parameterised over the node type @n@ (of kind @*->*->*@),\john{No explanation required?}
\label{haskell.firstuse.*->*->*}% automated use
...
call ``a basic block''.  
In the case of @CmmNode@, for example,
...
\john{We'd probably benefit from introducing this shorthand earlier, with nodes.}
We will often informally say ``a closed/closed block'' 
...
as @BUnit@ lifted a node to a block. We will soon return to the question
...
The excitement comes in the @GMany@ constructor. It has three fields:
\begin{itemize}
\item Every @Graph@ (whether open or closed) has a distinguished entry block,
where control always enters the block.  This distinguished block, closed at the exit,
 is the first argument of @GMany@.
\item The second argument represents an
arbitrary graph of blocks, each closed at both ends, and 
each labelled with a @BlockId@.
This graph, of type @BlockGraph n@, is represented as a finite map 
from @BlockId@ to a closed/closed @Block@.
\item The third field is called the ``tail'' of the @Graph@.  If
...
by @GNil@.  A one-block graph open at exit is represented by @GUnit@.
A one block graph closed at exit is represented by @(GMany b [] NoTail)@.
\label{haskell.firstuse.b}% automated use
A multi-block graph is represented by a @GMany@, with a @Tail@ or @NoTail@
depending on whether the graph is open or closed at exit.
...
gCat (GMany be bs (Tail bid bx)) (GUnit b2) 
   = GMany be bs (Tail bid (bx `BCat` b2))

gCat (GMany be1 bs1 (Tail bid bx1)) (GMany be2 bs2 bx2)
   = GMany be1 (addBlock bid (bx1 `BCat` be2) bs1 
                `unionBlocks` bs2) bx2
\end{code}}
\label{haskell.firstuse.unionBlocks}% automated use
\label{haskell.firstuse.addBlock}% automated use
\label{haskell.firstuse.bx2}% automated use
\label{haskell.firstuse.bs2}% automated use
\label{haskell.firstuse.be2}% automated use
\label{haskell.firstuse.bx1}% automated use
\label{haskell.firstuse.bs1}% automated use
\label{haskell.firstuse.be1}% automated use
\label{haskell.firstuse.bx}% automated use
\label{haskell.firstuse.bid}% automated use
\label{haskell.firstuse.be}% automated use
...
statically checked.  For example, in the last equation for @gCat@, the
first argument has type (@Graph@ @n@ @e@ @O@), so the third field of
@GMany@ must be of form (@Tail bid bx1@).  Moreover @bx1@ must be
...
All three entities (nodes, blocks, graphs) share some 
...
by an explicit branch to a label.
\end{itemize}
The body of a procedure would typically be represented by
an open/closed graph, where execution begins at the open end.
An alternative would be to represent a procedure body by a @BlockGraph@,
together with a @BlockId@ where execution begins.
...
example is shown in \figref{const-prop}.
\john{Code is slightly simplified from original; can probably go a bit farther.
  There are a few functions that are standard and should be reused throughout
  the compiler that I haven't shown in the figure.}
...
\end{code}
\caption{Representation of a dataflow lattice} \figlabel{const-prop}
\label{haskell.firstuse.WithTop}% automated use
\label{haskell.firstuse.Elt}% automated use
\label{haskell.firstuse.Top}% automated use
\label{haskell.firstuse.ConstFact}% automated use
\label{haskell.firstuse.Map}% automated use
\label{haskell.firstuse.Const}% automated use
\label{haskell.firstuse.constLattice}% automated use
\label{haskell.firstuse.fact:unbot}% automated use
\label{haskell.firstuse.Map.empty}% automated use
\label{haskell.firstuse..}% automated use
\label{haskell.firstuse.fact:unadd:unto}% automated use
\label{haskell.firstuse.stdMapJoin}% automated use
\label{haskell.firstuse.constFactAdd}% automated use
...
\label{haskell.firstuse.ch}% automated use
\label{haskell.firstuse.joined}% automated use
\label{haskell.firstuse.==}% automated use
\label{haskell.firstuse.NoChange}% automated use
\label{haskell.firstuse.SomeChange}% automated use
\label{haskell.firstuse.varHasConst}% automated use
\label{haskell.firstuse.ForwardTransfers}% automated use
\label{haskell.firstuse.lookupFact}% automated use
\label{haskell.firstuse.l}% automated use
\label{haskell.firstuse.Map.insert}% automated use
\label{haskell.firstuse.Cond}% automated use
\label{haskell.firstuse.tid}% automated use
\label{haskell.firstuse.fid}% automated use
\label{haskell.firstuse.I}% automated use
\label{haskell.firstuse.think}% automated use
\label{haskell.firstuse.the}% automated use
\label{haskell.firstuse.getInFacts}% automated use
\label{haskell.firstuse.might}% automated use
\label{haskell.firstuse.disappear}% automated use
\label{haskell.firstuse.with}% automated use
\label{haskell.firstuse.Hoopl4}% automated use
\label{haskell.firstuse.?}% automated use
\label{haskell.firstuse.constProp}% automated use
\label{haskell.firstuse.ForwardRewrites}% automated use
\label{haskell.firstuse.facts}% automated use
\label{haskell.firstuse.map:unEN}% automated use
\label{haskell.firstuse.map:unEE}% automated use
\label{haskell.firstuse.rewriteE}% automated use
\label{haskell.firstuse.return}% automated use
\label{haskell.firstuse.toAGraph}% automated use
\label{haskell.firstuse.>>=}% automated use
\label{haskell.firstuse.Var}% automated use
\label{haskell.firstuse.v}% automated use
\label{haskell.firstuse.M.lookup}% automated use
\label{haskell.firstuse.$}% automated use
\label{haskell.firstuse.Simplification}% automated use
\label{haskell.firstuse.simplify}% automated use
\label{haskell.firstuse.node}% automated use
\label{haskell.firstuse.s}% automated use
...
\label{haskell.firstuse.s:une}% automated use
\label{haskell.firstuse.Binop}% automated use
\label{haskell.firstuse.Add}% automated use
\label{haskell.firstuse.i1}% automated use
\label{haskell.firstuse.i2}% automated use
\label{haskell.firstuse.....}% automated use
\end{figure}
...
\label{haskell.firstuse.fact:unextend}% automated use
...
\label{haskell.firstuse.x.}% automated use
\label{haskell.firstuse.TailFactF}% automated use
\label{haskell.firstuse.BackwardTransfers}% automated use
\label{haskell.firstuse.TailFactB}% automated use
...
depend on the open/closed-ness of the node's exit.}
...
constant-propagation example.  The supplied transfer function should be 
\emph{monotonic} in the following sense: given a more informative fact in,
it should produce a more informative fact out.  Otherwise Hoopl might not
find a fixed point (although it will still terminate provided the join 
function of the lattice is monotonic).
\john{The last sentence is wrong. The fixpoint-finder only pushes facts
up the lattice (using the join operation). Only a finite number of steps
can be taken up the lattice. The particular input facts are completely irrelevant
as far as termination is concerned.}
\simon{How true! Yet a non-monotonic transfer function would be peculiar indeed.
Our algorithm will terminate, but it will not terminate with a fixed point.}
...
mkIfThenElse :: Expr -> AGraph CmmNode O O
             -> AGraph CmmNode O -> AGraph CmmNode O O
mkIfThenElse p t e
...
\label{haskell.firstuse.mkIfThenElse}% automated use
\label{haskell.firstuse.p}% automated use
\label{haskell.firstuse.withBlockIds}% automated use
\label{haskell.firstuse.l1}% automated use
\label{haskell.firstuse.l2}% automated use
\label{haskell.firstuse.l3}% automated use
\label{haskell.firstuse.:bs}% automated use
...
\label{haskell.firstuse.bg}% automated use
\label{haskell.firstuse.flattenG}% automated use
\label{haskell.firstuse.catNode}% automated use
\end{figure}
...
More precisely, the rewriter returns an @AGraph@ rather than a @Graph@.
An @AGraph@ is a function that is given 
...
rewriting is done.  
\john{Rewriting fuel appears out of the blue here, and I think we need to more clearly
point out that the goal is to provide an easy way to debug optimizations.
I'll take a hack at this section tomorrow when I'm less tired.}
(Because each rewrite leaves the observable behavior of the
program unchanged, it is safe to suppress rewrites at
any point.)
\john{This sentence takes as read a completely new idea. We need to
articulate our philosophy on how optimizations should be written (sequence of
simple, semantics-preserving transformations).}
...
for making new blocks; more precisely, the client-provided @ForwardRewrites@
...
Facts computed by the @analyseAndRewriteFwd@ depend on graphs produced by the rewrite
...
\john{Isn't this implied by ``observationally equivalent''?}
...
should debate.}
\john{I'm not sure which source file to look at, but I still maintain that this is
false. The argument for reaching a fixed point is clearly defined in terms of
the lattice; I don't see how the results of the analysis are relevant.}
...
\john{And remove the bogus parts: monad, one fn per analysis...}
...
analyseAndRewrite depth lattice tf rw
 ...
 where 
  ...
  gft_g_sh  = gftGraph lattice (gftBlock gft_n_sh)
  gft_g_rec = case depth of
                RewriteShallow -> gft_g_sh
                RewriteDeep    -> gft_g

  gft_n, gft_n_sh :: GFT_Node n f
  gft_n    = gftNodeRewrite tf rw gft_g_rec
...
\john{Inquisitive reader wants to know how analyseAndRewrite relates to
analyseAndRewriteFwd.}
...
\item The node-rewriting engine is instructs @gftNodeRewrite@
to use @gft_g_rec@ for recursive applications to rewritten graphs.
...
analysis.  However, it can still be constructed by applying the same functions
@gftGraph@ and @gftBlock@
