...
!assign.avail.2!  foldVarsDefd delFromAvail avail lhs
...
!avail.solve.1!  where soln = zdfSolveFwd  availVarsLattice 
...
\label{haskell.firstuse.CmmReg}% automated use
...
\T
\begin{code}
liveTransfers :: BackTransfers Middle Last Live
...
!live.zdfSolveBwd!   where soln = zdfSolveBwd  liveLattice liveTransfers emptyVarSet g
\end{code}%
\B
...
\label{haskell.firstuse.gen}% automated use
@kill@ functions.
\label{haskell.firstuse.kill}% automated use
...
\end{smallcode}
\label{haskell.def.gen}% automated definition
\label{haskell.def.kill}% automated definition
...
!insertLateReloads.1!  where result = zdfRewriteFwd RewriteShallow 
...
!deadRewrites.1!  where nothing _ _ = Nothing
        middleRemoveDeads :: Middle -> VarSet -> Maybe (Graph Middle Last)
...
!rewriteBwd.1!     where result = zdfRewriteBwd RewriteDeep 
...
!forward.sol.sig!forward_sol :: forall m l a . LastNode l => (forall b . Maybe b -> DFM a (Maybe b))
...
!set.last.*!     set_last (LastOutFacts l) = mapM_ (uncurry setFact) l
...
\label{haskell.firstuse.LastNode}% automated use
...
\label{haskell.firstuse.LastOutFacts}% automated use
...
\emph{open at the entry}.  \simon{I find ``enterable'' ambiguous,
...
\label{haskell.firstuse.NoRewrite}% automated use
