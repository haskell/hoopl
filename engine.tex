...
some moonotonicity property.  Something like: given a more informative
...
\simon{In this phase
it's guaranteed that no dataflow facts will change.}
\end{itemize}
...
is out of fuel. \simon{Or because the analysis has not yet reached a fixpoint,
so we discard?}
...
Is @DFMonad@ in the type of @zdfRewriteFwd@ the same as @DFM@ in this figure?}
...
I'm still not sure what it means, and the reader may be in a similar state.}
...
to get a list of reachable blocks in~@blocks@.  \simon{The only reader
to use postorder is to improve the speed of convergence.  Any enumeration
of the blocks will do for correctness.}
It then applies the higher-order @run@ function \simon{There must be
a bug here: @run@ is not bound.}, which repeatedly runs
