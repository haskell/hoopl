...
(see example in \secref{ckpoint-monad}).
To~avoid faulty rewrites,
any rewrite based on an invalid fact must be rolled back,
which means that transformations must be \emph{speculative}.
...
isolating faults in the optimizer (\secref{whalley-from-s2}).\remark{Perhaps say more
...
meet the constraints implied by @CkpointMonad@, as described in
...
\john{There's a relevant paragraph on the next page; not sure it belongs there}
...
described in \secref{ckpoint-monad}.
}
...
\hoopl's data types can support optimizations beyond classic
dataflow. 
For example, within GHC, \hoopl's  graphs are used 
...
Another example from Colin and Sukyoung: suppose that an optimization
wants to spawn some new function definitions?  LET'S GET THE SPECIFICS!
...
Several anonymous reviewers helped improve the
