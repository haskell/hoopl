...
+\ourlib's @NonLocal@ type class, which is defined in \figref{edges}.
 The @entryLabel@ method takes a first node (one closed on entry,
-\secref{nodes})
...
We~discuss $\rw$ in \secref{shallow-vs-deep}.
...
\subsection{Shape-dependent, node-independent analysis}

We noted in \secref{transfers} that
``\emph{the type of the transfer function's result 
depends on the shape of the node's exit}.''
This type is very convenient for clients:
because the client defines the node type,
the client also knows the shape of each node.
Therefore, a client can just write a transfer
function that matches on the node argument
and produces the right type of outgoing fact
for the shape of the node.

But what if you want to write a transfer function that
\emph{does not know the type of the node}?
For example, a dominator analysis
does not need to know the type of the node; it needs to know only about
the labels and edges in the graph.
Therefore, you might want to write a dominator analysis
that will work with \emph{any} type of node that is an
instance of the @Edges@ type class.
...
The client has defined auxiliary functions
\begin{smallcode}
`mapVE :: (Var  -> Maybe Expr) -> (Expr     -> Maybe Expr)
`mapEE :: (Expr -> Maybe Expr) -> (Expr     -> Maybe Expr)
`mapEN :: (Expr -> Maybe Expr) -> (Node e x -> Maybe (Node e x))
`mapVN :: (Var  -> Maybe Expr) -> (Node e x -> Maybe (Node e x))
