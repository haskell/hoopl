...
This paper is a revised version of a submission to ICFP'10. Comments
...
value constructors to shape is different for each node type~@n@, such
 functions cannot be polymorphic in both
the representation and the shape of nodes.
We~therefore, in our implementation, sacrifice polymorphism in shape:
a @FwdTransfer n f@ is represented by a \emph{triple} of functions,
each polymorphic in the node type but monomorphic in shape:
\begin{code}
...
work with triples; each client, which knows the node type, may supply either a triple
or a single function polymorphic in the shape of the node. 
...
\begin{smallfuzzcode}{6.6pt}
iterFwdRw 
  :: Monad m => FwdRewrite m n f -> FwdRewrite m n f
`iterFwdRw rw = wrapFRewrites' f rw
  where f rw' n f = liftM iter (rw' n f)
        iter NoFwdRes = NoFwdRes
        iter (FwdRes g rw2) =
          FwdRes g (rw2 `thenFwdRw` iterFwdRw rw)
\end{smallfuzzcode}
