...
As suggested by its type in \figref{api-types}, 
a @FwdRewrite@ is
a \emph{monadic} function, 
with monad~@m@ chosen by the client.
...
A~single dataflow fact might enable more than one rewrite.
\hoopl\ provides function @thenFwdRw@, which
combines two rewriting functions into one:
\verbatiminput{comb1}
...
rewriting function.
\hoopl\ provides
a function that makes a shallow rewrite deep:\finalremark{algebraic law wanted!}
\verbatiminput{iterf}
