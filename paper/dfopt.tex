...
the pure functional code is actually easier to write, and far easier
...
Designing good APIs is surprisingly
...
As~a concrete example, we consider constant propagation with constant
folding. 
...
Our first example \emph{interleaves} transformation and analysis.
...
a~return continuation, and exception handler, or what have you.
...
returns a @Node@ open on both entry and exit; the @Store@ node is
similar.  The types @`Var@ and @`Expr@ are private to the client, and
\ourlib{} knows nothing of them.  
Finally, the control-transfer nodes @Branch@ and @CondBranch@ are open on entry
and closed on exit.  
...
\emph{Explicit} edges between blocks are represented by the client's
node type;
an~explicit edge originates in a last node and flows to a first node.
...
How can \hoopl{} know these things, if it is polymorphic in the node type?
...
if a @Graph@ is closed on entry, it does not have a unique entry label.
%
...
\secref{fuel-monad}. 
%%  
%%  offer at least the operations of the @FuelMonad@ class (\figref{api-types}),
...
The client's model of how @analyzeAndRewriteFwdBody@ works is as follows:
...
Furthermore, the dataflow engine wants to know if
  $f_{\mathit{old}} \join f_{\mathit{new}} = f_{\mathit{old}}$,
...
the most useful is @extendJoinDomain@:
\begin{smallcode}
`extendJoinDomain
 :: (OldFact a -> NewFact a -> (ChangeFlag, WithTop a))
 -> (OldFact (WithTop a) -> NewFact (WithTop a)
                            -> (ChangeFlag, WithTop a))
\end{smallcode}
...
involving the @Top@ element obey all the appropriate algebraic laws.
\hoopl\ also provides a @`Bot@ constructor and type constructors
@`WithBot@, and @`WithTopAndBot@, along with similar functions.
@Top@ and @Bot@ are polymorphic, so for example,
@Top@~also has type @WithTopAndBot a@.
\john{The previous 3 paragraphs aren't worth their space.
NR: The issue is how to introduce @WithTop@ and @PElem@, which are used in
\figref{const-prop}.
I'd be happy if we could condense.}
...
\end{code}
The forward analysis starts with the bottom fact \{\} at every label.
...
A~transfer function has a \emph{dependent} type:
If the node is open on exit, the transfer function produces a single fact.
But if the node is \emph{closed} on exit,
the transfer function must
produce a collection of (@Label@,fact) pairs, one for each outgoing edge.  
%
The dependency can be expressed by Haskell's (recently added) 
...
comes out of a node
\emph{open} on exit is just a fact~@f@. The second declaration says that 
a @Fact C f@, which
comes out of a node
\emph{closed} on exit, is a mapping from @Label@ to facts.
...
A~programmer creating a rewrite function knows the type of a node~@n@ and
a dataflow fact~@f@.
A~rewrite function might also want to have access to a supply of fresh
names (e.g., to create new blocks) or to other state (e.g., a~mapping
...
When these choices are made, the easiest way to create a rewrite
...
node, fact, and (possibly) monad, but is polymorphic in the
...
In~the general case, it must be possible for rewriting to result in a graph.
...
\hoopl\ makes it easy for a client to use multiple rewrite functions;
...
rewriting function.
\john{We inconsistently use both ``rewriting function'' and ``rewrite function.''
We should pick one. I vote for the latter.}
...
\emph{the type of the transfer function's result 
depends on the shape of the node's exit}.
It~is easy for a client to write a dependent transfer function
...
and so can return a result appropriate to each node's shape.
But what if you want to write a transfer function that
\emph{does not know the type of the node}?

For example, a dominator analysis need not scrutinize nodes;
it needs to know only about
labels and edges in the graph.
We~prefer to write a dominator analysis that works
...
@n e x -> f -> Fact x f@,
because the only way to get the dependency right is to be able to
...
`mkFTransfer3 :: (n C O -> f -> Fact O f)
             ...
             -> FwdTransfer n f
\end{code}
We have used this interface to write a number functions that are
...
In the first case for assignment, a variable gets a constant value,
so we produce a dataflow fact mapping the variable to its value.
In the second case for assignment, the variable gets a non-constant value,
...
(Because function @lt@ scrutinizes a GADT, it cannot
...
\end{smallcode}
The client composes @mapXX@ functions
...
Applying @fmap nodeToG@ lifts the final node, if present, into a~@Graph@.
...
folding: @simplify@.
This function
rewrites constant expressions with their values
and rewrites a conditional branch on a
...
\hoopl\ requires that a client meet these preconditions:
\begin{itemize}
\item 
...
\end{smallfuzzcode}
Function @node@ uses @frewrite@ to extract the rewriting function from
@pass@, 
and applies the rewriting function to the node~@n@ and the incoming fact~@f@.
...
optimization fuel
finds an~$n$ such that program works correctly after $n-1$ rewrites
but fails 
after $n$~rewrites.
...
compiler writers.
\hoopl\ represents just one point in a very large design space, and 
even among ourselves we disagree about which nearby points might be
better.
How, then, should you evaluate \hoopl?

One way to evaluate \hoopl\ is by use cases.
...
Within GHC, \hoopl's API supports optimizations based on control flow
as well as on dataflow, such as eliminating branches to branches.
We have not contemplated trying
to extend Hoopl to establish or maintain SSA invariants.

Another way to evaluate \hoopl\ is to examine the API and its
implementation.  
Is~the design harmonious?
Will~the code be easy to maintain?
We~can't present enough information for
you to answer these questions, but we can draw your attention to
the aspects that seem most important.

One good sign about the API is that it supports many higher-order
combinators with simple, expressive types.
We~have had space to mention only a
few: 
@extendJoinDomain@, 
@thenFwdRw@, @deepFwdRw@, @shallowFwdRw@, and @pairFwd@.


The API's static encoding of open and closed shapes at compile time worked
out well,
...
@graph@, @cat@, @withFuel@, and @fixpoint@, we have clearly separated
multiple concerns:
...
But~even within this framework, there are 
alternative implementations;
for example, in addition to ``@concatMap@ style,'' we have also
written implementations in ``fold style'' and in continuation-passing
style.
Which implementation is preferred is a matter of taste, and of course
...
By~these means we have significantly improved the implementation of
...
Several anonymous reviewers provided helpful feedback, especially
reviewer~C, who gave us better language with which to describe our
work. 
