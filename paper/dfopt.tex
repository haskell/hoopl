...
to write correctly, than code that is mostly functional uses a mutable
...
\remark{Should we prefigure somehow the ``main goal'' of the
concluding section?}
...
\ourlib{} thereby offers a case study in the utility
of these features.
...
% at exits can transfer control (see also \secref{nonlocal-class}).
...
A~middle node, which is open at both entry and exit, transfers control
...
But if the node is \emph{closed} at exit,
...
reminiscent of @concatMap@, with the @block@ function
...
The @node@ function implements the interleaved rewrite-and-transfer
algorithm:
...
It uses @frewrite@ to extract the rewriting function from @pass@,
and applies it to the node~@n@ and the incoming fact~@f@.
Ignore the call to @withFuel@ for now; we discuss it next
in \secref{fuel}.  The result of the rewrite, @res@, is then
scrutinised by the @case@ expression.

In the @Nothing@ case, no rewrite takes place: either fuel is
...
on the open/closed shape of the node.  Each call of @node@ (in @block@,
\secref{block-impl}) has a statically-fixed open/closed shape, so
this use of @ShapeLifter@ is highly localised, and is specialised
...
The call to @withFuel@ in function @node@ allows us to set as
limit on the number of permitted rewrites.
This is useful when debugging:
when an optimizer produces a faulty program,
the blame could lie with any of many hundreds of rewrites.
To~find the first rewrite that 
transforms a program from working code to faulty code,
we use Whalley's
\citeyearpar{whalley:isolation} powerful fault-isolation technique.

Whalley's technique uses a limit on the number of rewrites
the optimizer may perform;
in~\hoopl, we call the limit
...
The throttling mechanism is isolated in a single function:
\begin{code}
`withFuel :: FuelMonad m => m (Maybe a) -> m (Maybe a)
\end{code}
If @withFuel@'s argument returns @Nothing@, \emph{or} if we have run out of
optimization fuel, @withFuel@ returns @Nothing@.
Otherwise, @withFuel@ consumes one unit of fuel and returns its
argument (which will be a @Just@).  
A~@FuelMonad@ is simply a state monad that provides operations
 @getFuel@ and @setFuel@, as shown in \figref{api-types}.
...
The code for @fixpoint@ is about 70 lines long;
for completeness, it
...
HOW FAST/SLOW ARE THE RESULTING OPTIMIZATIONS?
DO YOU TAKE A BIG HIT BECAUSE OF THE ABSTRACTIONS?
...
multiple interfaces for rewrite functions---a subtlety!
...
compile time worked out very well. 
Shapes may
