...
\simon{Actually it's a @FactBase@.}
%
...
and the client's transfer functions discriminate on the constructor
...
for constant
folding: @simplify@.
This function rewrites a conditional branch on a
...
The call to @withFuel@ in function @node@ enables us to 
set a limit on the number of nodes rewritten.
We~call the limit \emph{optimization fuel}.
...
Optimization fuel is used when debugging the optimizer:
...
consuming a unit of fuel if the argument is a @Just@.
If~fuel is exhaused, @withFuel@ @return@s @Nothing@, independent of
its argument.
