...
functions (\secref{transfer-function}): because a node or a block has
...
Function @node@ handles all rewriting,
but before we explain @node@'s implementation, 
we need to present a mechanism we use to~limit 
the number of rewrites that
are performed while optimizing a program.
We limit rewrites only when debugging an optimizer:
...
NEEDS THINKING ABOUT THE TRANSITION
...
\end{smallfuzzcode}
This code is more complicated than the code for @block@ or @graph@, but
it is still short.
%
