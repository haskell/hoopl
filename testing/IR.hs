...
data Proc = Proc { name :: String, args :: [Var], body :: Graph Node C C }
...
foldG _ z GNil = z
foldG f z (GUnit b) = foldB f z b
foldG f z (GMany {g_entry, g_blocks, g_exit}) =
  foldB f (foldl (foldB f) (foldExit (foldB f) z g_exit) g_blocks) g_entry

foldExit :: (a -> t C x -> a) -> a -> Exit t x -> a
foldExit _ z NoTail   = z
foldExit f z (Tail t) = f z t
...
showG b (GMany {g_entry, g_blocks, g_exit}) =
  showB b g_entry ++ concatMap (showB b) g_blocks ++ showExit (showB b) g_exit

showExit :: (n C x -> String) -> Exit n x -> String
showExit _ NoTail = ""
showExit p (Tail n) = p n
